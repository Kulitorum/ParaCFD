...
namespace paracfd::core
{
	...
	void TriangleBvh::build(const TriMesh& mesh, std::uint32_t leaf_size)
	{
		...
		for (std::uint32_t id = 0; id < mesh.triangle_count(); ++id)
		{
			...
			if (std::max({i0, i1, i2}) >= mesh.vertex_count()) continue;
	...
	ClosedMeshPointLocation TriangleBvh::classify_closed_point(Vec3d p,
		double boundary_tolerance) const
	{
		if(nodes_.empty())return ClosedMeshPointLocation::outside;
		const Aabb3d& shell=nodes_.front().bounds;
		const double diagonal=std::sqrt(length2(shell.hi-shell.lo));
		const double tolerance=boundary_tolerance>0.0?boundary_tolerance:
			64.0*std::numeric_limits<double>::epsilon()*std::max(1.0,diagonal);
		const NearestSurfacePoint closest=nearest(p,tolerance);
		if(closest.found&&closest.distance<=tolerance)
			return ClosedMeshPointLocation::boundary;

		auto ray_vote=[&](Vec3d direction)
		{
			direction=normalized(direction);
			const Vec3d centre=(shell.lo+shell.hi)*0.5;
			const double reach=4.0*(diagonal+std::sqrt(length2(p-centre))+1.0);
			const Vec3d delta=direction*reach;
			struct Crossing{double t=0.0;double orientation=0.0;};
			std::vector<Crossing> hits;
			std::vector<std::uint32_t> stack{0};
			while(!stack.empty())
			{
				const Node& node=nodes_[stack.back()];stack.pop_back();
				if(!segment_aabb(p,delta,node.bounds,0.0,1.0))continue;
				if(node.leaf())
				{
					for(std::uint32_t q=node.first;q<node.first+node.count;++q)
					{
						const BvhTriangle& triangle=primitives_[q].tri;
						double t=0.0,u=0.0,v=0.0;
						if(!segment_triangle(p,delta,triangle,
							std::max(1.0e-14,tolerance/reach),1.0,t,u,v))continue;
						const Vec3d normal=cross(triangle.b-triangle.a,triangle.c-triangle.a);
						hits.push_back({t,dot(normal,direction)});
					}
				}
			...
			const double group_tolerance=std::max(1.0e-12,8.0*tolerance/reach);
			int crossings=0;
			for(std::size_t first=0;first<hits.size();)
			{
				std::size_t last=first+1;
				while(last<hits.size()&&hits[last].t-hits[first].t<=group_tolerance)++last;
				bool positive=false,negative=false;
				for(std::size_t q=first;q<last;++q)
				{
					positive=positive||hits[q].orientation>0.0;
					negative=negative||hits[q].orientation<0.0;
				}
				// Mixed orientation at one geometric point is a tangential vertex/edge
				// contact. It does not cross the closed shell.
				if(positive!=negative)++crossings;
				first=last;
			}
			return (crossings&1)!=0;
		};

		const bool vote0=ray_vote({1.0,0.3713906763541037,0.6947465906068658});
		const bool vote1=ray_vote({-0.4385780260809998,1.0,0.2199274234618394});
		const bool vote2=ray_vote({0.2837127294733198,-0.5324471804789654,1.0});
		if(vote0==vote1||vote0==vote2)
			return vote0?ClosedMeshPointLocation::inside:ClosedMeshPointLocation::outside;
		if(vote1==vote2)
			return vote1?ClosedMeshPointLocation::inside:ClosedMeshPointLocation::outside;

		// This branch is practically unreachable with three Boolean votes, but keep a
		// winding fallback for future classifiers that may expose an ambiguous vote.
		double sum=0.0,compensation=0.0;
		for(const BvhTriangle& triangle:triangles_)
		{
			const Vec3d a=triangle.a-p,b=triangle.b-p,c=triangle.c-p;
			const double la=std::sqrt(length2(a)),lb=std::sqrt(length2(b)),lc=std::sqrt(length2(c));
			const double angle=2.0*std::atan2(dot(a,cross(b,c)),
				la*lb*lc+dot(a,b)*lc+dot(b,c)*la+dot(c,a)*lb);
			const double corrected=angle-compensation,next=sum+corrected;
			compensation=(next-sum)-corrected;sum=next;
		}
		return std::abs(sum)>2.0*std::acos(-1.0)?
			ClosedMeshPointLocation::inside:ClosedMeshPointLocation::outside;
	}
