...
// The interactive placement gizmo (paracfd-gui) fills M with a rotation·scale so a loaded model
// can be moved/rotated/scaled and then voxelized exactly where the user placed it.
//
// OCC-free + Qt-free (pure inline maths on a TriMesh bbox), so it lives in libparacfd's
