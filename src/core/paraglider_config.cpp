	...
	bool load_paraglider_config(const std::string& path, ParagliderConfig& out, std::string* error)
	{
		try
		{
			...
			if (auto it = root.find("amr"); it != root.end() && it->is_object())
			{
				...
				c.amr.max_levels = integer(*it, "max_levels", c.amr.max_levels); c.amr.topology_refinement_levels = integer(*it, "topology_refinement_levels", c.amr.topology_refinement_levels); c.amr.brick_size = integer(*it, "brick_size", c.amr.brick_size); c.amr.ghost_cells = integer(*it, "ghost_cells", c.amr.ghost_cells);
