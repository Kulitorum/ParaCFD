...
namespace paracfd::core
{
	...
	{
		if(wing.empty()||bvh.empty())throw std::invalid_argument("external aerodynamic core requires a placed fabric mesh");hierarchy_=AmrHierarchy::build_static(automatic_flow_domain(wing,config_.domain),wing,bvh,config_.amr);EmbeddedBoundaryBuildOptions options;options.min_volume_fraction=config_.amr.min_volume_fraction;options.min_aperture_area_fraction=config_.amr.min_aperture_area_fraction;options.complex_subdivisions=config_.amr.complex_subdivisions;embedded_boundary_=build_amr_embedded_boundary_atlas(hierarchy_,wing,bvh,options);if(!embedded_boundary_.ready_for_flow())throw std::runtime_error("external aerodynamic core has unresolved finest-level embedded-boundary topology");pressure_system_=build_composite_amr_pressure_system(hierarchy_,embedded_boundary_,true);fields_=std::make_unique<DeviceAmrFields>(hierarchy_);advection_=std::make_unique<DeviceAmrAdvection>(hierarchy_,bvh,2.5,&pressure_system_);projection_=std::make_unique<DeviceCompositeAmrProjection>(pressure_system_,*fields_);
	}
	...
	ExternalAeroStepStats ExternalAeroCore::step()
	{
		...
		const double regular_maximum=fields_->max_abs_velocity(),special_maximum=projection_->max_abs_special_velocity(),embedded_cfl_rate=projection_->max_embedded_cfl_rate(),h=hierarchy_.finest_cell_size();
		...
		const double cfl_rate=std::max({1e-9,std::abs(config_.freestream.speed)/h,regular_maximum/h,embedded_cfl_rate}),dt_value=config_.solver.cfl/cfl_rate,cfl_velocity=cfl_rate*h;const Real dt=static_cast<Real>(dt_value);
		const auto step_begin=std::chrono::steady_clock::now(),advection_begin=step_begin;advection_->advect(*fields_,dt);const auto advection_end=std::chrono::steady_clock::now();advection_->diffuse_smagorinsky(*fields_,static_cast<Real>(config_.freestream.nu),static_cast<Real>(config_.solver.smagorinsky_cs),dt);const auto turbulence_end=std::chrono::steady_clock::now();fields_->apply_external_aero_boundaries(static_cast<Real>(config_.freestream.speed));projection_->transport_embedded_apertures(dt,static_cast<Real>(config_.freestream.nu),static_cast<Real>(config_.solver.smagorinsky_cs),false);projection_->apply_smooth_fabric_wall_model(dt,static_cast<Real>(config_.freestream.nu));smooth_fabric_wall_applied_=true;const auto embedded_transport_end=std::chrono::steady_clock::now();ExternalAeroStepStats stats=project(true,dt_value);const auto step_end=std::chrono::steady_clock::now();
		stats.max_abs_regular_velocity=regular_maximum;stats.max_abs_special_velocity=special_maximum;stats.max_embedded_cfl_rate=embedded_cfl_rate;stats.max_abs_velocity=std::max(regular_maximum,special_maximum);stats.cfl_velocity=cfl_velocity;stats.effective_cfl=cfl_rate*dt_value;stats.advection_ms=elapsed_ms(advection_begin,advection_end);stats.turbulence_ms=elapsed_ms(advection_end,turbulence_end);stats.embedded_transport_ms=elapsed_ms(turbulence_end,embedded_transport_end);stats.les_applied=true;stats.embedded_transport_applied=true;stats.smooth_fabric_wall_applied=true;stats.gpu_step_ms=elapsed_ms(step_begin,step_end);if(stats.pressure.converged)physical_time_+=stats.dt;stats.physical_time=physical_time_;return stats;
	}
