...
namespace paracfd::core
{
		...
		:solver_(system),fields_(&fields)
	{
		...
		if(embedded_count_)
		{
			...
			embedded_carrier_count_=static_cast<int>(carrier_node.size());eb_node_a_=upload(node_a,"upload EB transport first nodes");eb_node_b_=upload(node_b,"upload EB transport second nodes");eb_negative_edge_=upload(negative_edge,"upload EB negative graph neighbours");eb_positive_edge_=upload(positive_edge,"upload EB positive graph neighbours");eb_carrier_node_=upload(carrier_node,"upload EB transport carrier nodes");eb_carrier_level_=upload(carrier_level,"upload EB transport carrier levels");eb_carrier_index_=upload(carrier_index,"upload EB transport carrier indices");eb_carrier_axis_=upload(carrier_axis,"upload EB transport carrier axes");eb_carrier_area_=upload(carrier_area,"upload EB transport carrier areas");eb_node_axis_sum_=allocate_zero<Real>(static_cast<std::size_t>(embedded_node_count_)*3,"allocate EB node component sums");eb_node_axis_weight_=allocate_zero<Real>(static_cast<std::size_t>(embedded_node_count_)*3,"allocate EB node component weights");eb_node_gradient_sum_=allocate_zero<Real>(static_cast<std::size_t>(embedded_node_count_)*9,"allocate EB node gradient sums");eb_node_gradient_weight_=allocate_zero<Real>(static_cast<std::size_t>(embedded_node_count_)*9,"allocate EB node gradient weights");eb_transport_length_=upload(transport_length,"upload EB aperture transport lengths");eb_transport_scratch_=allocate_zero<Real>(embedded_count_,"allocate EB aperture transport scratch");bytes_+=4*node_a.size()*sizeof(int)+2*carrier_node.size()*sizeof(int)+carrier_index.size()*sizeof(std::uint64_t)+carrier_axis.size()*sizeof(std::int8_t)+carrier_area.size()*sizeof(Real)+static_cast<std::size_t>(embedded_node_count_)*24*sizeof(Real)+static_cast<std::size_t>(embedded_count_)*2*sizeof(Real);
		}
	...
	{
		if(!(dt>Real(0))||molecular_nu<Real(0)||smagorinsky_cs<Real(0))throw std::invalid_argument("embedded aperture transport timestep/viscosity");if(!embedded_count_)return;const std::size_t node_components=static_cast<std::size_t>(embedded_node_count_)*3,node_gradients=static_cast<std::size_t>(embedded_node_count_)*9;check(cudaMemset(eb_node_axis_sum_,0,node_components*sizeof(Real)),"clear EB transport component sums");check(cudaMemset(eb_node_axis_weight_,0,node_components*sizeof(Real)),"clear EB transport component weights");check(cudaMemset(eb_node_gradient_sum_,0,node_gradients*sizeof(Real)),"clear EB transport gradient sums");check(cudaMemset(eb_node_gradient_weight_,0,node_gradients*sizeof(Real)),"clear EB transport gradient weights");if(embedded_carrier_count_)accumulate_eb_carrier_kernel<<<(embedded_carrier_count_+255)/256,256>>>(levels_,eb_carrier_node_,eb_carrier_level_,eb_carrier_index_,eb_carrier_axis_,eb_carrier_area_,eb_node_axis_sum_,eb_node_axis_weight_,embedded_carrier_count_);accumulate_eb_aperture_kernel<<<(embedded_count_+255)/256,256>>>(eb_node_a_,eb_node_b_,axis_,open_area_,special_velocity_,eb_node_axis_sum_,eb_node_axis_weight_,coarse_fine_count_,embedded_count_);accumulate_eb_gradient_kernel<<<(embedded_count_+255)/256,256>>>(eb_node_a_,eb_node_b_,direction_,axis_,open_area_,eb_transport_length_,eb_node_axis_sum_,eb_node_axis_weight_,eb_node_gradient_sum_,eb_node_gradient_weight_,coarse_fine_count_,embedded_count_);transport_eb_aperture_kernel<<<(embedded_count_+255)/256,256>>>(eb_node_a_,eb_node_b_,eb_negative_edge_,eb_positive_edge_,direction_,axis_,eb_transport_length_,special_velocity_,eb_node_axis_sum_,eb_node_axis_weight_,eb_node_gradient_sum_,eb_node_gradient_weight_,dt,molecular_nu,smagorinsky_cs,eb_transport_scratch_,coarse_fine_count_,embedded_count_);commit_eb_aperture_kernel<<<(embedded_count_+255)/256,256>>>(special_velocity_,eb_transport_scratch_,coarse_fine_count_,embedded_count_);check(cudaDeviceSynchronize(),"transport compact EB aperture velocities");
	}
