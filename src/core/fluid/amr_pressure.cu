...
namespace paracfd::core
{
	...
	namespace
	{
		...
		__global__ void accumulate_eb_gradient_kernel(const int* node_a,const int* node_b,const std::int8_t* direction,const std::int8_t* axis,const Real* area,const Real* transport_length,const Real* sum,const Real* weight,Real* gradient_sum,Real* gradient_weight,int special_offset,int count)
		{
			const int edge=blockIdx.x*blockDim.x+threadIdx.x;if(edge>=count)return;const int global=special_offset+edge,first=node_a[edge],second=node_b[edge],lower=direction[global]>0?first:second,upper=direction[global]>0?second:first,derivative_axis=axis[global];const Real d=max(transport_length[edge],Real(1e-12)),open_area=area[global];
			for(int component=0;component<3;++component){const Real lower_weight=weight[lower*3+component],upper_weight=weight[upper*3+component];if(!(lower_weight>Real(0)&&upper_weight>Real(0)))continue;const Real gradient=(sum[upper*3+component]/upper_weight-sum[lower*3+component]/lower_weight)/d,weighted=open_area*gradient;const int lower_slot=lower*9+component*3+derivative_axis,upper_slot=upper*9+component*3+derivative_axis;atomicAdd(gradient_sum+lower_slot,weighted);atomicAdd(gradient_sum+upper_slot,weighted);atomicAdd(gradient_weight+lower_slot,open_area);atomicAdd(gradient_weight+upper_slot,open_area);}
		}
		__global__ void transport_eb_aperture_kernel(const int* node_a,const int* node_b,const int* negative_edge,const int* positive_edge,const std::int8_t* direction,const std::int8_t* axis,const Real* transport_length,const Real* velocity,const Real* sum,const Real* weight,const Real* gradient_sum,const Real* gradient_weight,Real dt,Real molecular_nu,Real smagorinsky_cs,Real* output,Real* diffusion_rate,int special_offset,int count)
		{
			const int edge=blockIdx.x*blockDim.x+threadIdx.x;if(edge>=count)return;const int global=special_offset+edge,component=axis[global],first=node_a[edge],second=node_b[edge],lower=direction[global]>0?first:second,upper=direction[global]>0?second:first;const Real current=velocity[global],lower_weight=weight[lower*3+component],upper_weight=weight[upper*3+component],lower_value=lower_weight>Real(0)?sum[lower*3+component]/lower_weight:current,upper_value=upper_weight>Real(0)?sum[upper*3+component]/upper_weight:current,d=max(transport_length[edge],Real(1e-12));Real gradient[9],advector[3];for(int velocity_component=0;velocity_component<3;++velocity_component){const Real a_weight=weight[lower*3+velocity_component],b_weight=weight[upper*3+velocity_component];Real value=Real(0),samples=Real(0);if(a_weight>Real(0)){value+=sum[lower*3+velocity_component]/a_weight;samples+=Real(1);}if(b_weight>Real(0)){value+=sum[upper*3+velocity_component]/b_weight;samples+=Real(1);}advector[velocity_component]=samples>Real(0)?value/samples:Real(0);}advector[component]=current;for(int slot=0;slot<9;++slot){const Real a_weight=gradient_weight[lower*9+slot],b_weight=gradient_weight[upper*9+slot];Real value=Real(0),samples=Real(0);if(a_weight>Real(0)){value+=gradient_sum[lower*9+slot]/a_weight;samples+=Real(1);}if(b_weight>Real(0)){value+=gradient_sum[upper*9+slot]/b_weight;samples+=Real(1);}gradient[slot]=samples>Real(0)?value/samples:Real(0);}const Real normal_rate=abs(upper_value-lower_value)/d,strain=max(sqrt(max(Real(0),detail::compact_strain_magnitude_squared(gradient))),normal_rate),length=smagorinsky_cs*d,eddy_nu=length*length*strain;diffusion_rate[edge]=(molecular_nu+eddy_nu)/(d*d);const Real courant=min(Real(1),abs(current)*dt/d);const bool positive=current>=Real(0);const int upstream_edge=positive?negative_edge[edge]:positive_edge[edge],downstream_edge=positive?positive_edge[edge]:negative_edge[edge],upstream2_edge=upstream_edge>=0?(positive?negative_edge[upstream_edge]:positive_edge[upstream_edge]):-1;const bool complete=upstream_edge>=0&&upstream2_edge>=0&&downstream_edge>=0;const Real upstream=complete?velocity[special_offset+upstream_edge]:current,upstream2=complete?velocity[special_offset+upstream2_edge]:current,downstream=complete?velocity[special_offset+downstream_edge]:current;const Real normal_update=detail::bounded_compact_transport_update(current,lower_value,upper_value,upstream,upstream2,downstream,courant,Real(0),complete);output[edge]=detail::bounded_compact_tangential_update(normal_update,current,lower_value,upper_value,advector,gradient,component,dt);
		}
	...
	void DeviceCompositeAmrProjection::transport_embedded_apertures(Real dt,Real molecular_nu,Real smagorinsky_cs)
	{
		...
		check(cudaMemset(eb_node_gradient_sum_,0,node_gradients*sizeof(Real)),"clear EB transport gradient sums");
		check(cudaMemset(eb_node_gradient_weight_,0,node_gradients*sizeof(Real)),"clear EB transport gradient weights");
		...
		accumulate_eb_gradient_kernel<<<(embedded_count_+255)/256,256>>>(eb_node_a_,eb_node_b_,direction_,axis_,open_area_,eb_transport_length_,eb_node_axis_sum_,eb_node_axis_weight_,eb_node_gradient_sum_,eb_node_gradient_weight_,coarse_fine_count_,embedded_count_);
		transport_eb_aperture_kernel<<<(embedded_count_+255)/256,256>>>(eb_node_a_,eb_node_b_,eb_negative_edge_,eb_positive_edge_,direction_,axis_,eb_transport_length_,special_velocity_,eb_node_axis_sum_,eb_node_axis_weight_,eb_node_gradient_sum_,eb_node_gradient_weight_,dt,molecular_nu,smagorinsky_cs,eb_transport_scratch_,eb_diffusion_rate_,coarse_fine_count_,embedded_count_);
		...
		check(cudaDeviceSynchronize(),"transport compact EB aperture velocities");
	}
