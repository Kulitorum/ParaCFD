...
namespace paracfd::core
{
	...
	namespace
	{
		...
		__global__ void transport_eb_aperture_kernel(const int* node_a,const int* node_b,const std::int8_t* direction,const std::int8_t* axis,const Real* transport_length,const Real* velocity,const Real* sum,const Real* weight,Real dt,Real molecular_nu,Real* output,int special_offset,int count)
		{
			const int edge=blockIdx.x*blockDim.x+threadIdx.x;if(edge>=count)return;const int global=special_offset+edge,component=axis[global],first=node_a[edge],second=node_b[edge],lower=direction[global]>0?first:second,upper=direction[global]>0?second:first;const Real current=velocity[global],lower_weight=weight[lower*3+component],upper_weight=weight[upper*3+component],lower_value=lower_weight>Real(0)?sum[lower*3+component]/lower_weight:current,upper_value=upper_weight>Real(0)?sum[upper*3+component]/upper_weight:current,d=max(transport_length[edge],Real(1e-12));const Real courant=min(Real(1),abs(current)*dt/d),upstream=current>=Real(0)?lower_value:upper_value,diffusion=min(Real(0.25),molecular_nu*dt/(d*d));output[edge]=current+courant*(upstream-current)+diffusion*(lower_value+upper_value-Real(2)*current);
		}
	...
	{
		for(void* pointer:{(void*)levels_,(void*)active_,(void*)cut_face_mask_,(void*)volume_,(void*)integrated_,(void*)divergence_,(void*)rhs_,(void*)pressure_,(void*)first_dof_,(void*)second_dof_,(void*)direction_,(void*)axis_,(void*)open_area_,(void*)centre_distance_,(void*)special_velocity_,(void*)cf_fine_level_,(void*)cf_fine_index_,(void*)cf_group_,(void*)cf_group_level_,(void*)cf_group_index_,(void*)cf_group_axis_,(void*)cf_group_area_,(void*)cf_group_sum_,(void*)eb_node_a_,(void*)eb_node_b_,(void*)eb_carrier_node_,(void*)eb_carrier_level_,(void*)eb_carrier_index_,(void*)eb_carrier_axis_,(void*)eb_carrier_area_,(void*)eb_node_axis_sum_,(void*)eb_node_axis_weight_,(void*)eb_transport_length_,(void*)eb_transport_scratch_})if(pointer)cudaFree(pointer);
	}
