...
namespace paracfd::core
{
	...
	namespace
	{
		...
		__global__ void compute_smooth_wall_patch_delta_kernel(const int* node,const Real* normal,
			const Real* area,const Real* distance,const Real* sum,const Real* weight,
			Real dt,Real molecular_nu,Real* velocity_delta,Real* force_per_density,int count)
		{
			const int patch=blockIdx.x*blockDim.x+threadIdx.x;if(patch>=count)return;const int target=node[patch];Real velocity[3];for(int component=0;component<3;++component){const int slot=target*3+component;velocity[component]=weight[slot]>Real(0)?sum[slot]/weight[slot]:Real(0);}const Real* n=normal+3*patch;const Real un=velocity[0]*n[0]+velocity[1]*n[1]+velocity[2]*n[2],tangent[3]={velocity[0]-un*n[0],velocity[1]-un*n[1],velocity[2]-un*n[2]},speed=sqrt(max(Real(0),tangent[0]*tangent[0]+tangent[1]*tangent[1]+tangent[2]*tangent[2]));if(!(speed>Real(0)))return;const Real friction=detail::smooth_wall_friction_velocity(speed,distance[patch],molecular_nu),traction=friction*friction/speed;for(int component=0;component<3;++component){const int slot=target*3+component;if(!(weight[slot]>Real(0)))continue;const Real force=area[patch]*traction*tangent[component];force_per_density[3*patch+component]=force;atomicAdd(velocity_delta+slot,-dt*force/weight[slot]);}
		}
	...
	DeviceCompositeAmrPressureSolver::DeviceCompositeAmrPressureSolver(const CompositeAmrPressureSystem& system):op_(system),n_(system.storage_size)
	{
		...
		for(const CoarseFinePressureConnection& edge:system.coarse_fine)add_coarse_edge(edge.coarse_dof,edge.fine_dof,edge.open_area*pressure_gradient_factor(edge));for(const CoarseFinePressureConnection& edge:system.embedded)add_coarse_edge(edge.coarse_dof,edge.fine_dof,edge.open_area*pressure_gradient_factor(edge));for(const CompositePressureGauge& gauge:system.gauges){const int coarse=system.preconditioner_aggregate[gauge.dof]-base_offset_;if(coarse>=0&&coarse<base_cells_)coarse_diagonal_double[coarse]+=gauge.coefficient;}
		...
		:solver_(system),fields_(&fields)
	{
		...
		std::vector<CoarseFinePressureConnection> special=system.coarse_fine;special.insert(special.end(),system.embedded.begin(),system.embedded.end());std::vector<int> first(special_count_),second(special_count_);std::vector<std::int8_t> direction(special_count_),axis(special_count_);std::vector<Real> area(special_count_),distance(special_count_),gradient_factor(special_count_);for(int edge=0;edge<special_count_;++edge){first[edge]=special[edge].coarse_dof;second[edge]=special[edge].fine_dof;direction[edge]=special[edge].direction;axis[edge]=special[edge].axis;area[edge]=static_cast<Real>(special[edge].open_area);distance[edge]=static_cast<Real>(special[edge].centre_distance);gradient_factor[edge]=static_cast<Real>(pressure_gradient_factor(special[edge]));}first_dof_=upload(first,"upload composite flux first DOFs");second_dof_=upload(second,"upload composite flux second DOFs");direction_=upload(direction,"upload composite flux direction");axis_=upload(axis,"upload composite flux axes");open_area_=upload(area,"upload composite flux areas");centre_distance_=upload(distance,"upload composite flux distances");pressure_gradient_factor_=upload(gradient_factor,"upload composite flux pressure-gradient factors");special_velocity_=allocate_zero<Real>(special_count_,"allocate composite special velocity");max_abs_scratch_=allocate_zero<Real>(1,"allocate special-velocity maximum");
		...
		if(!system.surface_patches.empty())
		{
			...
			fabric_wall_carrier_count_=static_cast<int>(carrier_node.size());fabric_wall_patch_count_=static_cast<int>(wall_patch_node.size());fabric_wall_unique_carrier_count_=static_cast<int>(unique_carrier_level.size());wall_edge_node_a_=upload(wall_edge_a,"upload fabric wall first nodes");wall_edge_node_b_=upload(wall_edge_b,"upload fabric wall second nodes");wall_carrier_node_=upload(carrier_node,"upload fabric wall carrier nodes");wall_carrier_level_=upload(carrier_level,"upload fabric wall carrier levels");wall_carrier_index_=upload(carrier_index,"upload fabric wall carrier indices");wall_carrier_axis_=upload(carrier_axis,"upload fabric wall carrier axes");wall_carrier_mass_=upload(carrier_mass,"upload fabric wall carrier masses");wall_unique_carrier_level_=upload(unique_carrier_level,"upload unique fabric wall carrier levels");wall_unique_carrier_index_=upload(unique_carrier_index,"upload unique fabric wall carrier indices");wall_unique_carrier_axis_=upload(unique_carrier_axis,"upload unique fabric wall carrier axes");wall_unique_carrier_mass_=upload(unique_carrier_mass,"upload unique fabric wall carrier masses");wall_patch_node_=upload(wall_patch_node,"upload smooth-wall patch nodes");wall_patch_normal_=upload(wall_patch_normal,"upload smooth-wall patch normals");wall_patch_area_=upload(wall_patch_area,"upload smooth-wall patch areas");wall_patch_distance_=upload(wall_patch_distance,"upload smooth-wall patch distances");wall_patch_force_per_density_=allocate_zero<Real>(static_cast<std::size_t>(fabric_wall_patch_count_)*3,"allocate smooth-wall patch forces");wall_momentum_scratch_=allocate_zero<double>(3,"allocate fabric wall momentum reduction");wall_node_rate_=upload(wall_rate,"upload fabric wall viscous rates");wall_node_axis_sum_=allocate_zero<Real>(static_cast<std::size_t>(fabric_wall_node_count_)*3,"allocate fabric wall velocity sums");wall_node_axis_weight_=allocate_zero<Real>(static_cast<std::size_t>(fabric_wall_node_count_)*3,"allocate fabric wall velocity weights");wall_node_velocity_delta_=allocate_zero<Real>(static_cast<std::size_t>(fabric_wall_node_count_)*3,"allocate fabric wall velocity deltas");bytes_+=2*wall_edge_a.size()*sizeof(int)+2*carrier_node.size()*sizeof(int)+carrier_index.size()*sizeof(std::uint64_t)+carrier_axis.size()*sizeof(std::int8_t)+carrier_mass.size()*sizeof(Real)+wall_patch_node.size()*sizeof(int)+(8*wall_patch_node.size()+static_cast<std::size_t>(fabric_wall_node_count_)*10)*sizeof(Real)+3*sizeof(double)+unique_carrier_level.size()*sizeof(int)+unique_carrier_index.size()*sizeof(std::uint64_t)+unique_carrier_axis.size()*sizeof(std::int8_t)+unique_carrier_mass.size()*sizeof(Real);
		}
	...
	{
		if(!(dt>Real(0))||!(molecular_nu>Real(0)))throw std::invalid_argument("smooth fabric wall timestep/viscosity");if(!fabric_wall_node_count_||!fabric_wall_patch_count_)return;const std::size_t components=static_cast<std::size_t>(fabric_wall_node_count_)*3;check(cudaMemset(wall_node_axis_sum_,0,components*sizeof(Real)),"clear smooth-wall velocity sums");check(cudaMemset(wall_node_axis_weight_,0,components*sizeof(Real)),"clear smooth-wall velocity weights");check(cudaMemset(wall_node_velocity_delta_,0,components*sizeof(Real)),"clear smooth-wall velocity deltas");check(cudaMemset(wall_patch_force_per_density_,0,static_cast<std::size_t>(fabric_wall_patch_count_)*3*sizeof(Real)),"clear smooth-wall patch forces");if(embedded_count_)accumulate_wall_aperture_velocity_kernel<<<(embedded_count_+255)/256,256>>>(wall_edge_node_a_,wall_edge_node_b_,axis_,open_area_,pressure_gradient_factor_,special_velocity_,wall_node_axis_sum_,wall_node_axis_weight_,coarse_fine_count_,embedded_count_);if(fabric_wall_carrier_count_)accumulate_wall_carrier_velocity_kernel<<<(fabric_wall_carrier_count_+255)/256,256>>>(levels_,wall_carrier_node_,wall_carrier_level_,wall_carrier_index_,wall_carrier_axis_,wall_carrier_mass_,wall_node_axis_sum_,wall_node_axis_weight_,fabric_wall_carrier_count_);compute_smooth_wall_patch_delta_kernel<<<(fabric_wall_patch_count_+255)/256,256>>>(wall_patch_node_,wall_patch_normal_,wall_patch_area_,wall_patch_distance_,wall_node_axis_sum_,wall_node_axis_weight_,dt,molecular_nu,wall_node_velocity_delta_,wall_patch_force_per_density_,fabric_wall_patch_count_);if(embedded_count_)apply_no_slip_wall_to_aperture_kernel<<<(embedded_count_+255)/256,256>>>(wall_edge_node_a_,wall_edge_node_b_,axis_,wall_node_velocity_delta_,special_velocity_,coarse_fine_count_,embedded_count_);if(fabric_wall_carrier_count_)apply_no_slip_wall_to_carrier_kernel<<<(fabric_wall_carrier_count_+255)/256,256>>>(levels_,wall_carrier_node_,wall_carrier_level_,wall_carrier_index_,wall_carrier_axis_,wall_node_velocity_delta_,fabric_wall_carrier_count_);check(cudaDeviceSynchronize(),"apply smooth fabric wall model");
	}
