...
namespace paracfd::core
{
		...
		:solver_(system),fields_(&fields)
	{
		...
		if(!system.surface_patches.empty())
		{
			...
			std::vector<Real> wall_rate(fabric_wall_node_count_,Real(0));for(const auto& patch:system.surface_patches){const double normal_length=std::sqrt(length2(patch.normal));if(!(patch.area>=0&&normal_length>0))throw std::runtime_error("invalid fabric wall patch geometry");for(int dof:{patch.plus_dof,patch.minus_dof}){auto found=wall_node_map.find(dof);if(found==wall_node_map.end())continue;const double volume=system.volume[dof],distance=std::abs(dot(system.centroid[dof]-patch.centroid,patch.normal))/normal_length,scale=std::cbrt(std::max(0.0,volume));if(!(volume>0&&distance>std::max(1e-14,scale*1e-10)))throw std::runtime_error("fabric wall patch has zero same-side centroid distance");wall_rate[found->second]+=static_cast<Real>(patch.area/(volume*distance));}}
