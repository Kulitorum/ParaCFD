...
namespace paracfd::core
{
	...
	namespace
	{
		...
		__global__ void compute_smooth_wall_patch_delta_kernel(const int* node,const Real* normal,
			const Real* area_over_volume,const Real* distance,const Real* sum,const Real* weight,
			Real dt,Real molecular_nu,Real* velocity_delta,int count)
		{
			const int patch=blockIdx.x*blockDim.x+threadIdx.x;if(patch>=count)return;const int target=node[patch];Real velocity[3];for(int component=0;component<3;++component){const int slot=target*3+component;velocity[component]=weight[slot]>Real(0)?sum[slot]/weight[slot]:Real(0);}const Real* n=normal+3*patch;const Real un=velocity[0]*n[0]+velocity[1]*n[1]+velocity[2]*n[2],tx=velocity[0]-un*n[0],ty=velocity[1]-un*n[1],tz=velocity[2]-un*n[2],speed=sqrt(max(Real(0),tx*tx+ty*ty+tz*tz));if(!(speed>Real(0)))return;const Real friction=detail::smooth_wall_friction_velocity(speed,distance[patch],molecular_nu),scale=-dt*area_over_volume[patch]*friction*friction/speed;atomicAdd(velocity_delta+target*3,scale*tx);atomicAdd(velocity_delta+target*3+1,scale*ty);atomicAdd(velocity_delta+target*3+2,scale*tz);
		}
	...
	void DeviceCompositeAmrProjection::transport_embedded_apertures(Real dt,Real molecular_nu,Real smagorinsky_cs)
	{
		...
		if(fabric_wall_node_count_&&molecular_nu>Real(0))
		{
	...
	{
		if(!(dt>Real(0))||!(molecular_nu>Real(0)))throw std::invalid_argument("smooth fabric wall timestep/viscosity");if(!fabric_wall_node_count_||!fabric_wall_patch_count_)return;const std::size_t components=static_cast<std::size_t>(fabric_wall_node_count_)*3;check(cudaMemset(wall_node_axis_sum_,0,components*sizeof(Real)),"clear smooth-wall velocity sums");check(cudaMemset(wall_node_axis_weight_,0,components*sizeof(Real)),"clear smooth-wall velocity weights");check(cudaMemset(wall_node_velocity_delta_,0,components*sizeof(Real)),"clear smooth-wall velocity deltas");if(embedded_count_)accumulate_wall_aperture_velocity_kernel<<<(embedded_count_+255)/256,256>>>(wall_edge_node_a_,wall_edge_node_b_,axis_,open_area_,pressure_gradient_factor_,special_velocity_,wall_node_axis_sum_,wall_node_axis_weight_,coarse_fine_count_,embedded_count_);if(fabric_wall_carrier_count_)accumulate_wall_carrier_velocity_kernel<<<(fabric_wall_carrier_count_+255)/256,256>>>(levels_,wall_carrier_node_,wall_carrier_level_,wall_carrier_index_,wall_carrier_axis_,wall_carrier_mass_,wall_node_axis_sum_,wall_node_axis_weight_,fabric_wall_carrier_count_);compute_smooth_wall_patch_delta_kernel<<<(fabric_wall_patch_count_+255)/256,256>>>(wall_patch_node_,wall_patch_normal_,wall_patch_area_over_volume_,wall_patch_distance_,wall_node_axis_sum_,wall_node_axis_weight_,dt,molecular_nu,wall_node_velocity_delta_,fabric_wall_patch_count_);if(embedded_count_)apply_no_slip_wall_to_aperture_kernel<<<(embedded_count_+255)/256,256>>>(wall_edge_node_a_,wall_edge_node_b_,axis_,wall_node_velocity_delta_,special_velocity_,coarse_fine_count_,embedded_count_);if(fabric_wall_carrier_count_)apply_no_slip_wall_to_carrier_kernel<<<(fabric_wall_carrier_count_+255)/256,256>>>(levels_,wall_carrier_node_,wall_carrier_level_,wall_carrier_index_,wall_carrier_axis_,wall_node_velocity_delta_,fabric_wall_carrier_count_);check(cudaDeviceSynchronize(),"apply smooth fabric wall model");
	}
