...
namespace scour::core
{
	...
	SCOUR_HD inline double nezu_urms(double us, double z, double h, double g) { return g * 2.30 * us * exp(-z / h); }
	SCOUR_HD inline double nezu_vrms(double us, double z, double h, double g) { return g * 1.63 * us * exp(-z / h); }
	SCOUR_HD inline double nezu_wrms(double us, double z, double h, double g) { return g * 1.27 * us * exp(-z / h); }
	// shear stress −u'w' = u*²(1−z/h) ⇒ R13 = <u'w'> = −u*²(1−z/h), scaled by γ².
	SCOUR_HD inline double nezu_uw(double us, double z, double h, double g) { return -(g * g) * us * us * (1.0 - z / h); }

	// Lower-triangular Cholesky of R_ij (research/14 §3): a21=a32=0.
	SCOUR_HD inline void sem_cholesky(double us, double z, double h, double g,
	...
	SCOUR_HD inline double sem_tent(double s) { double a = fabs(s); return a < 1.0 ? 1.2247448713915890 * (1.0 - a) : 0.0; }

	// Convected SEM sums S_j(y,z) = (1/√N) Σ_k ε_j^k f_σ(0−x_k, y−y_k, z−z_k).
	SCOUR_HD inline void sem_sums(double y, double z, const double* ex, const double* ey, const double* ez,
	...
	SCOUR_HD inline double sem_boxvol(double sigma, double Ly, double Lz)
