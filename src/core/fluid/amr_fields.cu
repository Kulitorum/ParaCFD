...
namespace paracfd::core
{
	...
	DeviceAmrFields::DeviceAmrFields(const AmrHierarchy& h)
	{
		...
		for(std::size_t l=0;l<levels_.size();++l)
		{
			...
			d.u=alloc<Real>(d.brick_count*d.layout.u_stride);d.v=alloc<Real>(d.brick_count*d.layout.v_stride);d.w=alloc<Real>(d.brick_count*d.layout.w_stride);d.p=alloc<Real>(d.brick_count*d.layout.cell_stride);d.nut=alloc<Real>(d.brick_count*d.layout.cell_stride);d.temp=alloc<Real>(d.brick_count*d.layout.cell_stride);
	...
	void DeviceAmrFields::fill(Real v){for(auto& d:levels_)if(d.brick_count){auto run=[&](Real* p,std::size_t n){int block=256;fill_kernel<<<static_cast<unsigned>((n+block-1)/block),block>>>(p,n,v);};run(d.u,d.brick_count*d.layout.u_stride);run(d.v,d.brick_count*d.layout.v_stride);run(d.w,d.brick_count*d.layout.w_stride);run(d.p,d.brick_count*d.layout.cell_stride);run(d.nut,d.brick_count*d.layout.cell_stride);run(d.temp,d.brick_count*d.layout.cell_stride);}check(cudaDeviceSynchronize(),"fill AMR fields");}
		...
		for (std::size_t level = 0; level < hierarchy.levels().size(); ++level)
		{
			...
			for (std::size_t i = 0; i < bricks.size(); ++i)
			{
				...
				bricks[i] = {static_cast<float>(brick.origin.x), static_cast<float>(brick.origin.y),
					static_cast<float>(brick.origin.z), brick.h, brick.flags};
			}
			...
			views[level] = {allocation.lookup, allocation.bricks, source.lookup_mask, static_cast<int>(source.lookup.size()), static_cast<int>(source.bricks.size()), source.h};
