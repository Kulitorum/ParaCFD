...
namespace paracfd::core
{
		...
		const TriMesh& mesh, const TriangleBvh& bvh, const EmbeddedBoundaryBuildOptions& options)
	{
		...
		for(int level=0;level<static_cast<int>(hierarchy.levels().size());++level)
		{
			...
			if(!found)continue;const double h=source.h,brick_width=bs*h;const Vec3d origin=hierarchy.domain().lo+Vec3d{minimum.x*brick_width-h,minimum.y*brick_width-h,minimum.z*brick_width-h};const UniformEbGrid grid{origin,(maximum.x-minimum.x+1)*bs+2,(maximum.y-minimum.y+1)*bs+2,(maximum.z-minimum.z+1)*bs+2,h};
