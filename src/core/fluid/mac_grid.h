...
namespace windcfd::core
{
	...
	struct MacGrid
	{
		...
		WINDCFD_HD double dx(int i) const { return dxa ? dxa[i] : h; }
		WINDCFD_HD double dy(int j) const { return dya ? dya[j] : h; }
		WINDCFD_HD double dz(int k) const { return dza ? dza[k] : h; }
		// Cell-centre world coordinate.
		WINDCFD_HD double xc(int i) const { return xca ? xca[i] : (i + 0.5) * h; }
		WINDCFD_HD double yc(int j) const { return yca ? yca[j] : (j + 0.5) * h; }
		WINDCFD_HD double zc(int k) const { return zca ? zca[k] : (k + 0.5) * h; }
		// Face (staggered-node) world coordinate: i in [0,nx].
		WINDCFD_HD double xf(int i) const { return xfa ? xfa[i] : i * h; }
		WINDCFD_HD double yf(int j) const { return yfa ? yfa[j] : j * h; }
		WINDCFD_HD double zf(int k) const { return zfa ? zfa[k] : k * h; }
		// Domain extent per axis (terminal cumulative face coordinate, NOT n*h on a graded grid).
		WINDCFD_HD double Lx() const { return xfa ? xfa[nx] : nx * h; }
		WINDCFD_HD double Ly() const { return yfa ? yfa[ny] : ny * h; }
		WINDCFD_HD double Lz() const { return zfa ? zfa[nz] : nz * h; }
		...
		WINDCFD_HD double h_min() const { return hmin > 0.0 ? hmin : h; }
		...
		WINDCFD_HD double dxc(int i) const { return xca ? (xca[i] - xca[i - 1]) : h; }
		WINDCFD_HD double dyc(int j) const { return yca ? (yca[j] - yca[j - 1]) : h; }
		WINDCFD_HD double dzc(int k) const { return zca ? (zca[k] - zca[k - 1]) : h; }
		...
		WINDCFD_HD double gapx(int i) const { if (i < 0) return dx(0); if (i > nx - 2) return dx(nx - 1); return xc(i + 1) - xc(i); }
		WINDCFD_HD double gapy(int j) const { if (j < 0) return dy(0); if (j > ny - 2) return dy(ny - 1); return yc(j + 1) - yc(j); }
		WINDCFD_HD double gapz(int k) const { if (k < 0) return dz(0); if (k > nz - 2) return dz(nz - 1); return zc(k + 1) - zc(k); }
	};
	...
	WINDCFD_HD inline double d2_axis(double fm, double c, double fp, double hL, double hR)
	{
	...
	WINDCFD_HD inline double locate_frac(const double* node, int N, double x)
	{
		int lo = 0, hi = N - 1;
		while (hi - lo > 1)
		{
	...
	WINDCFD_HD inline double grid_fx(MacGrid g, double x) { return g.xfa ? locate_frac(g.xfa, g.nx + 1, x) : x / g.h; }
	WINDCFD_HD inline double grid_fy(MacGrid g, double y) { return g.yfa ? locate_frac(g.yfa, g.ny + 1, y) : y / g.h; }
	WINDCFD_HD inline double grid_fz(MacGrid g, double z) { return g.zfa ? locate_frac(g.zfa, g.nz + 1, z) : z / g.h; }
	WINDCFD_HD inline double grid_cx(MacGrid g, double x) { return g.xca ? locate_frac(g.xca, g.nx, x) : x / g.h - 0.5; }
	WINDCFD_HD inline double grid_cy(MacGrid g, double y) { return g.yca ? locate_frac(g.yca, g.ny, y) : y / g.h - 0.5; }
	WINDCFD_HD inline double grid_cz(MacGrid g, double z) { return g.zca ? locate_frac(g.zca, g.nz, z) : z / g.h - 0.5; }
