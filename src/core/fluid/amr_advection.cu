...
namespace paracfd::core
{
		...
		const CompositeAmrPressureSystem& system,DeviceAmrFields& fields):system_(&system),fields_(&fields)
	{
		...
		coarse_fine_count_=static_cast<int>(system.coarse_fine.size());std::vector<int> coarse_fine_a(coarse_fine_count_),coarse_fine_b(coarse_fine_count_);std::vector<Real> coarse_fine_area(coarse_fine_count_);for(int edge=0;edge<coarse_fine_count_;++edge){const auto& connection=system.coarse_fine[edge];coarse_fine_a[edge]=connection.direction>0?connection.coarse_dof:connection.fine_dof;coarse_fine_b[edge]=connection.direction>0?connection.fine_dof:connection.coarse_dof;coarse_fine_area[edge]=static_cast<Real>(connection.open_area);}
		...
		x_=allocate<Real>(storage_size_,"allocate composite cell momentum x");y_=allocate<Real>(storage_size_,"allocate composite cell momentum y");z_=allocate<Real>(storage_size_,"allocate composite cell momentum z");delta_x_=allocate<Real>(storage_size_,"allocate composite cell momentum dx");delta_y_=allocate<Real>(storage_size_,"allocate composite cell momentum dy");delta_z_=allocate<Real>(storage_size_,"allocate composite cell momentum dz");volume_=upload(volume,"upload composite cell momentum volumes");active_=upload(system.active,"upload composite cell momentum active mask");cut_face_mask_=upload(system.cut_face_mask,"upload composite cell momentum cut-face mask");compact_plus_mask_=upload(compact_plus,"upload composite cell momentum compact ownership");regular_velocity_=allocate<Real>(regular_count_,"allocate composite cell momentum perimeter flux");regular_a_=upload(regular_a,"upload composite cell momentum perimeter lower nodes");regular_b_=upload(regular_b,"upload composite cell momentum perimeter upper nodes");regular_area_=upload(regular_area,"upload composite cell momentum perimeter areas");coarse_fine_a_=upload(coarse_fine_a,"upload composite cell momentum coarse/fine lower nodes");coarse_fine_b_=upload(coarse_fine_b,"upload composite cell momentum coarse/fine upper nodes");coarse_fine_area_=upload(coarse_fine_area,"upload composite cell momentum coarse/fine areas");embedded_a_=upload(embedded_a,"upload composite cell momentum aperture lower nodes");embedded_b_=upload(embedded_b,"upload composite cell momentum aperture upper nodes");embedded_area_=upload(embedded_area,"upload composite cell momentum aperture areas");bytes_=static_cast<std::size_t>(7*storage_size_+2*regular_count_+coarse_fine_count_+embedded_count_)*sizeof(Real)+static_cast<std::size_t>(3*storage_size_)*sizeof(unsigned char)+static_cast<std::size_t>(2*regular_count_+2*coarse_fine_count_+2*embedded_count_)*sizeof(int);
	}

	DeviceCompositeCellMomentumTransport::~DeviceCompositeCellMomentumTransport()
	{
		for(void* pointer:{(void*)x_,(void*)y_,(void*)z_,(void*)delta_x_,(void*)delta_y_,(void*)delta_z_,(void*)volume_,(void*)regular_velocity_,(void*)active_,(void*)cut_face_mask_,(void*)compact_plus_mask_,(void*)coarse_fine_a_,(void*)coarse_fine_b_,(void*)coarse_fine_area_,(void*)embedded_a_,(void*)embedded_b_,(void*)embedded_area_,(void*)regular_a_,(void*)regular_b_,(void*)regular_area_})if(pointer)cudaFree(pointer);
	}
