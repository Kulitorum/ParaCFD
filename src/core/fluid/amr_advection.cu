...
namespace paracfd::core
{
	namespace
	{
		...
		{
			const DeviceAmrFieldLevelView view=views[level],forward=forward_views[level];const int bs=view.layout.brick_size,per_component=bs*bs*(bs+1),q=blockIdx.x*blockDim.x+threadIdx.x,total=view.brick_count*3*per_component;if(q>=total)return;int component,brick,i,j,k;decode_face_work(view,q,component,brick,i,j,k);if(view.flags[brick]&BRICK_COVERED)return;const GpuBrickRecord record=hierarchy.levels[level].bricks[brick];const GpuAmrPoint point=face_node_point(record,component,i,j,k);const std::size_t index=component==0?view.layout.u_index(brick,i,j,k):(component==1?view.layout.v_index(brick,i,j,k):view.layout.w_index(brick,i,j,k));const Real current=component==0?view.u[index]:(component==1?view.v[index]:view.w[index]),phf=component==0?forward.u[index]:(component==1?forward.v[index]:forward.w[index]);const unsigned char links=component==0?links_u[index]:(component==1?links_v[index]:links_w[index]);Real result=phf;if(!(links&0x40)){Real mn,mx;const GpuAmrPoint back=rk2_trace(hierarchy,views,level,record,brick,point,dt);sample_finest_bounded(hierarchy,views,component,back,current,&mn,&mx);const GpuAmrPoint forward_point=rk2_trace(hierarchy,views,level,record,brick,point,-dt);const Real phn=sample_finest(hierarchy,forward_views,component,forward_point,phf);result=max(mn,min(mx,phf+Real(0.5)*(current-phn)));}if(component==0)u_out[index]=result;else if(component==1)v_out[index]=result;else w_out[index]=result;
		}
