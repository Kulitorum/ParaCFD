...
namespace paracfd::core
{
	namespace
	{
		...
		__device__ Real sample_local(GpuAmrHierarchyView hierarchy,const DeviceAmrFieldLevelView* views,int level,const GpuBrickRecord& record,int component,int brick,GpuAmrPoint point)
		{
			...
			Real y=(Real(point.y)-Real(record.origin_y))/Real(record.h)-(component==1?Real(0):Real(0.5));
			Real z=(Real(point.z)-Real(record.origin_z))/Real(record.h)-(component==2?Real(0):Real(0.5));
			const int i0=static_cast<int>(floor(x)),j0=static_cast<int>(floor(y)),k0=static_cast<int>(floor(z));x-=i0;y-=j0;z-=k0;Real result=0;const int bs=views[level].layout.brick_size;
		...
		{
			point.x=fminf(hierarchy.domain_hi.x-1e-5f,fmaxf(hierarchy.domain_lo.x+1e-5f,point.x));point.y=fminf(hierarchy.domain_hi.y-1e-5f,fmaxf(hierarchy.domain_lo.y+1e-5f,point.y));point.z=fminf(hierarchy.domain_hi.z-1e-5f,fmaxf(hierarchy.domain_lo.z+1e-5f,point.z));const GpuBrickLocation location=locate_finest(hierarchy,point);if(location.brick<0)return fallback;return sample_local(hierarchy,views,location.level,hierarchy.levels[location.level].bricks[location.brick],component,location.brick,point);
		}
		__global__ void advect_level_kernel(GpuAmrHierarchyView hierarchy,const DeviceAmrFieldLevelView* views,int level,Real dt,const unsigned char* protect_u,const unsigned char* protect_v,const unsigned char* protect_w,Real* u_out,Real* v_out,Real* w_out)
		{
			const DeviceAmrFieldLevelView view=views[level];const int bs=view.layout.brick_size,per_component=bs*bs*(bs+1),q=blockIdx.x*blockDim.x+threadIdx.x,total=view.brick_count*3*per_component;if(q>=total)return;int r=q,local=r%per_component;r/=per_component;const int component=r%3,brick=r/3;if(view.flags[brick]&BRICK_COVERED)return;int i=0,j=0,k=0;if(component==0){i=local%(bs+1);local/=bs+1;j=local%bs;k=local/bs;}else if(component==1){i=local%bs;local/=bs;j=local%(bs+1);k=local/(bs+1);}else{i=local%bs;local/=bs;j=local%bs;k=local/bs;}const GpuBrickRecord record=hierarchy.levels[level].bricks[brick];GpuAmrPoint point{record.origin_x+(i+(component==0?0.0f:0.5f))*record.h,record.origin_y+(j+(component==1?0.0f:0.5f))*record.h,record.origin_z+(k+(component==2?0.0f:0.5f))*record.h};const std::size_t index=component==0?view.layout.u_index(brick,i,j,k):(component==1?view.layout.v_index(brick,i,j,k):view.layout.w_index(brick,i,j,k));const Real current=component==0?view.u[index]:(component==1?view.v[index]:view.w[index]);const unsigned char protected_face=component==0?protect_u[index]:(component==1?protect_v[index]:protect_w[index]);Real result=current;if(!protected_face){const Real ux=sample_local(hierarchy,views,level,record,0,brick,point),uy=sample_local(hierarchy,views,level,record,1,brick,point),uz=sample_local(hierarchy,views,level,record,2,brick,point);GpuAmrPoint back{point.x-static_cast<float>(dt*ux),point.y-static_cast<float>(dt*uy),point.z-static_cast<float>(dt*uz)};result=sample_finest(hierarchy,views,component,back,current);}if(component==0)u_out[index]=result;else if(component==1)v_out[index]=result;else w_out[index]=result;
		}
