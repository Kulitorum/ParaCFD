...
namespace paracfd::core
{
	...
	{
		if(!system.hierarchy||system.hierarchy->levels().size()!=1||fields.levels().size()!=1||!system.coarse_fine.empty())throw std::invalid_argument("composite cell momentum CPU reference currently requires one uniform level");if(!(dt>0)||!(freestream_speed>=0)||embedded_velocity.size()!=system.embedded.size()||state.x.size()!=static_cast<std::size_t>(system.storage_size)||state.y.size()!=state.x.size()||state.z.size()!=state.x.size())throw std::invalid_argument("invalid composite cell momentum CPU state");const AmrHierarchy& hierarchy=*system.hierarchy;const AmrLevel& metadata=hierarchy.levels()[0];const AmrHostLevelFields& values=fields.levels()[0];const int bs=system.brick_size,cells=bs*bs*bs,offset=system.level_offset[0];const double area=metadata.h*metadata.h;std::vector<double> dx(system.storage_size),dy(system.storage_size),dz(system.storage_size);const std::vector<CompositeEbMomentumRegularConnection> compact_regular=build_composite_eb_momentum_regular_connections(system);std::vector<unsigned char> compact_plus(system.storage_size,0);for(const auto& connection:compact_regular)if(connection.lower_dof>=0&&connection.lower_dof<system.storage_size)compact_plus[connection.lower_dof]|=static_cast<unsigned char>(1u<<connection.face.component);
		...
		auto face_value=[&](int brick,int axis,int i,int j,int k){if(axis==0)return static_cast<double>(values.u[values.layout.u_index(brick,i+1,j,k)]);if(axis==1)return static_cast<double>(values.v[values.layout.v_index(brick,i,j+1,k)]);return static_cast<double>(values.w[values.layout.w_index(brick,i,j,k+1)]);};
	...
	{
		if(!system.hierarchy||system.hierarchy->levels().size()!=1||fields.level_count()!=1||!system.coarse_fine.empty())throw std::invalid_argument("composite cell momentum transport currently requires one uniform level");storage_size_=system.storage_size;level_offset_=system.level_offset[0];const int bs=system.brick_size;base_cell_count_=static_cast<int>(system.hierarchy->levels()[0].bricks.size())*bs*bs*bs;if(storage_size_<=0||base_cell_count_<=0||level_offset_<0||level_offset_+base_cell_count_>storage_size_)throw std::invalid_argument("invalid composite cell momentum storage layout");active_host_=system.active;volume_host_=system.volume;std::vector<Real> volume(storage_size_);for(int q=0;q<storage_size_;++q){if(system.active[q]&&!(system.volume[q]>0))throw std::invalid_argument("active composite cell momentum node has non-positive volume");volume[q]=static_cast<Real>(system.volume[q]);}
		const std::vector<CompositeEbMomentumRegularConnection> regular=build_composite_eb_momentum_regular_connections(system);regular_count_=static_cast<int>(regular.size());std::vector<int> regular_a(regular_count_),regular_b(regular_count_);std::vector<Real> regular_area(regular_count_);std::vector<AmrMacFaceAddress> regular_faces;regular_faces.reserve(regular_count_);std::vector<unsigned char> compact_plus(storage_size_,0);for(int edge=0;edge<regular_count_;++edge){regular_a[edge]=regular[edge].lower_dof;regular_b[edge]=regular[edge].upper_dof;regular_area[edge]=static_cast<Real>(regular[edge].open_area);regular_faces.push_back(regular[edge].face);if(regular_a[edge]>=0&&regular_a[edge]<storage_size_)compact_plus[regular_a[edge]]|=static_cast<unsigned char>(1u<<regular[edge].face.component);}regular_face_map_=std::make_unique<DeviceAmrMacFaceMap>(fields,regular_faces);
		embedded_count_=static_cast<int>(system.embedded.size());std::vector<int> embedded_a(embedded_count_),embedded_b(embedded_count_);std::vector<Real> embedded_area(embedded_count_);for(int edge=0;edge<embedded_count_;++edge){const auto& connection=system.embedded[edge];embedded_a[edge]=connection.direction>0?connection.coarse_dof:connection.fine_dof;embedded_b[edge]=connection.direction>0?connection.fine_dof:connection.coarse_dof;embedded_area[edge]=static_cast<Real>(connection.open_area);}
		x_=allocate<Real>(storage_size_,"allocate composite cell momentum x");y_=allocate<Real>(storage_size_,"allocate composite cell momentum y");z_=allocate<Real>(storage_size_,"allocate composite cell momentum z");delta_x_=allocate<Real>(storage_size_,"allocate composite cell momentum dx");delta_y_=allocate<Real>(storage_size_,"allocate composite cell momentum dy");delta_z_=allocate<Real>(storage_size_,"allocate composite cell momentum dz");volume_=upload(volume,"upload composite cell momentum volumes");active_=upload(system.active,"upload composite cell momentum active mask");cut_face_mask_=upload(system.cut_face_mask,"upload composite cell momentum cut-face mask");compact_plus_mask_=upload(compact_plus,"upload composite cell momentum compact ownership");regular_velocity_=allocate<Real>(regular_count_,"allocate composite cell momentum perimeter flux");regular_a_=upload(regular_a,"upload composite cell momentum perimeter lower nodes");regular_b_=upload(regular_b,"upload composite cell momentum perimeter upper nodes");regular_area_=upload(regular_area,"upload composite cell momentum perimeter areas");embedded_a_=upload(embedded_a,"upload composite cell momentum aperture lower nodes");embedded_b_=upload(embedded_b,"upload composite cell momentum aperture upper nodes");embedded_area_=upload(embedded_area,"upload composite cell momentum aperture areas");bytes_=static_cast<std::size_t>(7*storage_size_+regular_count_+regular_count_+embedded_count_)*sizeof(Real)+static_cast<std::size_t>(3*storage_size_)*sizeof(unsigned char)+static_cast<std::size_t>(2*regular_count_+2*embedded_count_)*sizeof(int);
	}

	DeviceCompositeCellMomentumTransport::~DeviceCompositeCellMomentumTransport()
	{
		for(void* pointer:{(void*)x_,(void*)y_,(void*)z_,(void*)delta_x_,(void*)delta_y_,(void*)delta_z_,(void*)volume_,(void*)regular_velocity_,(void*)active_,(void*)cut_face_mask_,(void*)compact_plus_mask_,(void*)embedded_a_,(void*)embedded_b_,(void*)embedded_area_,(void*)regular_a_,(void*)regular_b_,(void*)regular_area_})if(pointer)cudaFree(pointer);
	}
	...
	{
		if((embedded_count_&&!embedded_velocity)||!(dt>Real(0))||!(freestream_speed>=Real(0)))throw std::invalid_argument("invalid composite cell momentum timestep");check(cudaMemset(delta_x_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell momentum dx");check(cudaMemset(delta_y_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell momentum dy");check(cudaMemset(delta_z_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell momentum dz");const DeviceAmrFieldLevelView view=fields_->level_view(0);const Real h=static_cast<Real>(system_->hierarchy->levels()[0].h);transport_regular_cell_momentum_kernel<<<(base_cell_count_+255)/256,256>>>(view,h,level_offset_,active_,cut_face_mask_,compact_plus_mask_,x_,y_,z_,dt,external_aero,freestream_speed,delta_x_,delta_y_,delta_z_);if(regular_count_){regular_face_map_->gather(regular_velocity_);transport_compact_cell_momentum_kernel<<<(regular_count_+255)/256,256>>>(regular_a_,regular_b_,regular_area_,regular_velocity_,x_,y_,z_,dt,delta_x_,delta_y_,delta_z_,regular_count_);}if(embedded_count_)transport_compact_cell_momentum_kernel<<<(embedded_count_+255)/256,256>>>(embedded_a_,embedded_b_,embedded_area_,embedded_velocity,x_,y_,z_,dt,delta_x_,delta_y_,delta_z_,embedded_count_);apply_cell_momentum_kernel<<<(storage_size_+255)/256,256>>>(active_,volume_,delta_x_,delta_y_,delta_z_,x_,y_,z_,storage_size_);check(cudaDeviceSynchronize(),"composite cell momentum transport");
	}
