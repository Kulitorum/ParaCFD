...
namespace paracfd::core
{
	namespace
	{
		...
		__global__ void smooth_wall_surface_cell_impulse_kernel(const int* dof,
			...
			const Real* z,Real dt,Real molecular_nu,Real* dx,Real* dy,Real* dz,
			Real* force_per_density,int count)
		{
			const int patch=blockIdx.x*blockDim.x+threadIdx.x;if(patch>=count)return;const int node=dof[patch];const Real* n=normal+3*patch,velocity[3]={x[node],y[node],z[node]},un=velocity[0]*n[0]+velocity[1]*n[1]+velocity[2]*n[2],tangent[3]={velocity[0]-un*n[0],velocity[1]-un*n[1],velocity[2]-un*n[2]},speed=sqrt(max(Real(0),tangent[0]*tangent[0]+tangent[1]*tangent[1]+tangent[2]*tangent[2]));if(!(speed>Real(0)))return;const Real friction=detail::smooth_wall_friction_velocity(speed,distance[patch],molecular_nu),traction=friction*friction/speed,force[3]={area[patch]*traction*tangent[0],area[patch]*traction*tangent[1],area[patch]*traction*tangent[2]};for(int component=0;component<3;++component)force_per_density[3*patch+component]=force[component];atomicAdd(dx+node,-dt*force[0]);atomicAdd(dy+node,-dt*force[1]);atomicAdd(dz+node,-dt*force[2]);
		}
	...
	{
		if(!(molecular_viscosity>0)||!(dt>0)||!(density>0)||state.x.size()!=static_cast<std::size_t>(system.storage_size)||state.y.size()!=state.x.size()||state.z.size()!=state.x.size())throw std::invalid_argument("invalid composite cell smooth-wall state");std::vector<double> dx(system.storage_size),dy(system.storage_size),dz(system.storage_size);std::vector<SmoothFabricWallPatchLoad> loads;loads.reserve(2*system.surface_patches.size());for(const CompositeSurfacePressurePatch& patch:system.surface_patches){const double normal_length=std::sqrt(length2(patch.normal));if(!(patch.area>=0&&normal_length>0))throw std::runtime_error("invalid composite cell smooth-wall patch");const Vec3d unit=patch.normal/normal_length;for(int dof:{patch.plus_dof,patch.minus_dof}){if(dof<0||dof>=system.storage_size||!system.active[dof]||!(system.volume[dof]>0))throw std::runtime_error("invalid composite cell smooth-wall side");const double distance=std::abs(dot(system.centroid[dof]-patch.centroid,unit)),scale=std::cbrt(system.volume[dof]);if(!(distance>std::max(1e-14,scale*1e-10)))throw std::runtime_error("composite cell smooth-wall side has zero distance");const Real vx=state.x[dof],vy=state.y[dof],vz=state.z[dof],nx=static_cast<Real>(unit.x),ny=static_cast<Real>(unit.y),nz=static_cast<Real>(unit.z),un=vx*nx+vy*ny+vz*nz,tx=vx-un*nx,ty=vy-un*ny,tz=vz-un*nz,speed=std::sqrt(std::max(Real(0),tx*tx+ty*ty+tz*tz));Real fx=0,fy=0,fz=0;if(speed>Real(0)){const Real friction=detail::smooth_wall_friction_velocity(speed,static_cast<Real>(distance),static_cast<Real>(molecular_viscosity)),traction=friction*friction/speed,area=static_cast<Real>(patch.area);fx=area*traction*tx;fy=area*traction*ty;fz=area*traction*tz;dx[dof]-=dt*static_cast<double>(fx);dy[dof]-=dt*static_cast<double>(fy);dz[dof]-=dt*static_cast<double>(fz);}loads.push_back({patch.source_triangle_id,patch.source_face_id,patch.centroid,{density*static_cast<double>(fx),density*static_cast<double>(fy),density*static_cast<double>(fz)}});}}for(int q=0;q<system.storage_size;++q)if(system.active[q]){const double inverse=1.0/system.volume[q];state.x[q]=static_cast<Real>(static_cast<double>(state.x[q])+dx[q]*inverse);state.y[q]=static_cast<Real>(static_cast<double>(state.y[q])+dy[q]*inverse);state.z[q]=static_cast<Real>(static_cast<double>(state.z[q])+dz[q]*inverse);}return loads;
	}
	...
	{
		if(!(dt>Real(0))||!(molecular_viscosity>Real(0)))throw std::invalid_argument("invalid composite cell smooth-wall timestep");if(!surface_count_)return;check(cudaMemset(delta_x_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell smooth-wall dx");check(cudaMemset(delta_y_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell smooth-wall dy");check(cudaMemset(delta_z_,0,static_cast<std::size_t>(storage_size_)*sizeof(Real)),"clear composite cell smooth-wall dz");check(cudaMemset(surface_wall_force_per_density_,0,static_cast<std::size_t>(surface_count_)*3*sizeof(Real)),"clear composite cell smooth-wall forces");smooth_wall_surface_cell_impulse_kernel<<<(surface_count_+255)/256,256>>>(surface_dof_,surface_normal_,surface_area_,surface_distance_,x_,y_,z_,dt,molecular_viscosity,delta_x_,delta_y_,delta_z_,surface_wall_force_per_density_,surface_count_);apply_cell_momentum_kernel<<<(storage_size_+255)/256,256>>>(active_,volume_,delta_x_,delta_y_,delta_z_,x_,y_,z_,storage_size_);check(cudaDeviceSynchronize(),"apply composite cell smooth-wall impulse");
	}
