...
namespace paracfd::core
{
	namespace
	{
		...
		__global__ void accumulate_smooth_wall_matrix_kernel(const int* dof,
		...
		{
			const int patch=blockIdx.x*blockDim.x+threadIdx.x;if(patch>=count)return;const int node=dof[patch];const Real* n=normal+3*patch,velocity[3]={x[node],y[node],z[node]},un=velocity[0]*n[0]+velocity[1]*n[1]+velocity[2]*n[2],tangent[3]={velocity[0]-un*n[0],velocity[1]-un*n[1],velocity[2]-un*n[2]},speed=sqrt(max(Real(0),tangent[0]*tangent[0]+tangent[1]*tangent[1]+tangent[2]*tangent[2]));Real k=Real(0);if(speed>Real(0)){const Real friction=detail::smooth_wall_friction_velocity(speed,distance[patch],molecular_nu);k=area[patch]*friction*friction/speed;}coefficient[patch]=k;if(!(k>Real(0)))return;Real* a=matrix+6*node;atomicAdd(a,k*(Real(1)-n[0]*n[0]));atomicAdd(a+1,-k*n[0]*n[1]);atomicAdd(a+2,-k*n[0]*n[2]);atomicAdd(a+3,k*(Real(1)-n[1]*n[1]));atomicAdd(a+4,-k*n[1]*n[2]);atomicAdd(a+5,k*(Real(1)-n[2]*n[2]));
		}
		__global__ void apply_implicit_smooth_wall_kernel(const unsigned char* active,
			const Real* volume,const Real* matrix,Real dt,Real* x,Real* y,Real* z,int count)
		{
			const int q=blockIdx.x*blockDim.x+threadIdx.x;if(q>=count||!active[q])return;const Real scale=dt/volume[q],*k=matrix+6*q,a00=Real(1)+scale*k[0],a01=scale*k[1],a02=scale*k[2],a11=Real(1)+scale*k[3],a12=scale*k[4],a22=Real(1)+scale*k[5],l00=sqrt(a00),l10=a01/l00,l20=a02/l00,l11=sqrt(a11-l10*l10),l21=(a12-l20*l10)/l11,l22=sqrt(a22-l20*l20-l21*l21),r0=x[q],r1=y[q],r2=z[q],v0=r0/l00,v1=(r1-l10*v0)/l11,v2=(r2-l20*v0-l21*v1)/l22,n2=v2/l22,n1=(v1-l21*n2)/l11,n0=(v0-l10*n1-l20*n2)/l00;x[q]=n0;y[q]=n1;z[q]=n2;
		}
	...
	{
		if(!(dt>Real(0))||!(molecular_viscosity>Real(0)))throw std::invalid_argument("invalid composite cell smooth-wall timestep");if(!surface_count_)return;check(cudaMemset(wall_matrix_,0,static_cast<std::size_t>(storage_size_)*6*sizeof(Real)),"clear implicit composite cell smooth-wall matrices");accumulate_smooth_wall_matrix_kernel<<<(surface_count_+255)/256,256>>>(surface_dof_,surface_normal_,surface_area_,surface_distance_,x_,y_,z_,molecular_viscosity,wall_matrix_,surface_wall_coefficient_,surface_count_);apply_implicit_smooth_wall_kernel<<<(storage_size_+255)/256,256>>>(active_,volume_,wall_matrix_,dt,x_,y_,z_,storage_size_);finalize_smooth_wall_force_kernel<<<(surface_count_+255)/256,256>>>(surface_dof_,surface_normal_,x_,y_,z_,surface_wall_coefficient_,surface_wall_force_per_density_,surface_count_);check(cudaDeviceSynchronize(),"apply implicit composite cell smooth-wall impulse");
	}
