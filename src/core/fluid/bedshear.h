...
namespace paracfd::core
{
	...
	PARACFD_HD inline double wall_ks(double d50) { return 2.5 * d50; }        // research/04 §3
	PARACFD_HD inline double wall_z0_grain(double d50) { return d50 / 12.0; } // = ks/30

	// Guarded rough inversion: u* = κ·U_p/ln(z_p/z0), clamp ln arg ≥ e so u* ≤ κ·U_p
	// (research/04 §6 guard). z0 must be > 0.
	PARACFD_HD inline double wall_ustar_rough(double Up, double zp, double z0, double kappa)
	{
		double arg = zp / z0;
	...
	PARACFD_HD inline double wall_z0_cj(double ustar, double ks, double nu)
	{
	...
	PARACFD_HD inline double wall_ustar(double Up, double zp, double d50, double nu, double kappa, int regime, int cj_iters)
	{
	...
	PARACFD_HD inline double wall_zp(double h, double ks) { return fmax(1.5 * h, 2.0 * ks); }
	...
	PARACFD_HD inline int cavity_branch(double Hc, double h, double zp0, double Ugap, double nu)
	{
	...
	PARACFD_HD inline double cavity_tau_gap_turb(double Ugap, double Hc, double z0, double rho, double kappa)
	{
	...
	PARACFD_HD inline double cavity_tau_gap_laminar(double Ugap, double Hc, double rho, double nu)
	{
	...
	PARACFD_HD inline void bed_sample_uv(const double* u, const double* v, MacGrid g, int i, int j, double zp, double& uc, double& vc)
	{
