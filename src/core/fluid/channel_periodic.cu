...
namespace paracfd::core
{
	namespace
	{
		...
		PARACFD_HD inline double ucenter(const double* u, MacGrid g, int i, int j, int k)
		{ return 0.5 * (u[g.uidx(i, j, k)] + u[g.uidx(i + 1, j, k)]); }
		PARACFD_HD inline double vcenter(const double* v, MacGrid g, int i, int j, int k)
		{ return 0.5 * (v[g.vidx(i, j, k)] + v[g.vidx(i, j + 1, k)]); }

		// horizontal speed magnitude at cell (i,j,k) with free-slip mirror in z.
		PARACFD_HD inline double hspeed(const double* u, const double* v, MacGrid g, int i, int j, int k)
		{
		...
		PARACFD_HD inline double mixlen_cell(const double* u, const double* v, MacGrid g, double kappa, double Lz, int i, int j, int k)
		{
			double z = (k + 0.5) * g.h;
		...
		PARACFD_HD inline double zdiff_u(const double* uIn, const double* nut, MacGrid g, double dt, double nu, int i, int j, int k)
		{
		...
		PARACFD_HD inline double zdiff_v(const double* vIn, const double* nut, MacGrid g, double dt, double nu, int i, int j, int k)
		{
