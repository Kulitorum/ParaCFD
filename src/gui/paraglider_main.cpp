...
{
	std::string config,step,screenshot;int autoclose_ms=0;double thin_y_fraction=-1,thin_y_width=0.125,sweep_min=0,sweep_max=0,sweep_step=1;bool offscreen=false,thin_y_width_explicit=false,conservative_momentum=true,half_wing=false,sweep_requested=false;
	...
	paracfd::gui::ParagliderWindow window;window.show();if(thin_y_fraction>=0||thin_y_width_explicit)window.setThinYDiagnostic(thin_y_fraction>=0?thin_y_fraction:0.5,thin_y_width);window.setConservativeMomentum(conservative_momentum);bool ready=false;if(!config.empty()){ready=window.loadConfigFile(QString::fromStdString(config),false);if(half_wing)window.setHalfWingSimulation(true);if(ready)ready=sweep_requested?window.startAoaSweep(sweep_min,sweep_max,sweep_step):window.buildGrid();}else if(!step.empty()){if(half_wing)window.setHalfWingSimulation(true);ready=window.loadStepFile(QString::fromStdString(step),true);if(ready)ready=sweep_requested?window.startAoaSweep(sweep_min,sweep_max,sweep_step):window.buildGrid();}else{window.loadLastFile();if(half_wing)window.setHalfWingSimulation(true);if(sweep_requested)ready=window.startAoaSweep(sweep_min,sweep_max,sweep_step);}
