...
{
	std::string config,step,screenshot;int autoclose_ms=0;double thin_y_fraction=-1;bool offscreen=false;
	...
	paracfd::gui::ParagliderWindow window;window.show();if(thin_y_fraction>=0)window.setThinYDiagnostic(thin_y_fraction);bool ready=false;if(!config.empty())ready=window.loadConfigFile(QString::fromStdString(config),true);else if(!step.empty()){ready=window.loadStepFile(QString::fromStdString(step),true)&&window.buildGrid();}
