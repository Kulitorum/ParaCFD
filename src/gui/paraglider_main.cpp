...
int main(int argc,char** argv)
{
	...
	const int result=app.exec();const bool expected_auto_pause_missing=exit_after_auto_pause&&!sweep_requested&&ready&&!window.simulationAutoPaused();const long long final_steps=window.steps(),final_frames=window.viewer()?window.viewer()->framesRendered():0;const double final_time=window.physicalTime();window.close();const double seconds=autoclose_ms/1000.0;std::fprintf(stderr,"[paraglider] exit: %lld steps, t=%.6f s, %lld frames%s\n",final_steps,final_time,final_frames,autoclose_ms>0?" (scripted smoke)":"");if((autoclose_ms>0&&ready&&final_steps==0)||(exit_after_sweep&&sweep_requested&&!ready)||(exit_after_auto_pause&&!sweep_requested&&!ready)||auto_pause_exit_failed||expected_auto_pause_missing)return 3;(void)seconds;return result;
