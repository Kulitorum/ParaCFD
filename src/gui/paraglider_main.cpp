...
{
	std::string config,step,screenshot;int autoclose_ms=0;double thin_y_fraction=-1,thin_y_width=0.125;bool offscreen=false,thin_y_width_explicit=false;
	...
	paracfd::gui::ParagliderWindow window;window.show();if(thin_y_fraction>=0||thin_y_width_explicit)window.setThinYDiagnostic(thin_y_fraction>=0?thin_y_fraction:0.5,thin_y_width);bool ready=false;if(!config.empty())ready=window.loadConfigFile(QString::fromStdString(config),true);else if(!step.empty()){ready=window.loadStepFile(QString::fromStdString(step),true)&&window.buildGrid();}
