...
{
	std::string config,step,screenshot;int autoclose_ms=0;bool offscreen=false;
	...
	paracfd::gui::ParagliderWindow window;window.show();bool ready=false;if(!config.empty())ready=window.loadConfigFile(QString::fromStdString(config),true);else if(!step.empty()){ready=window.loadStepFile(QString::fromStdString(step),true)&&window.buildGrid();}
	if(autoclose_ms>0){if(ready)window.startSimulation();if(!screenshot.empty())QTimer::singleShot(std::max(100,autoclose_ms-500),&app,[&window,screenshot]{if(window.viewer())window.viewer()->grabFramebuffer().save(QString::fromStdString(screenshot));});QTimer::singleShot(autoclose_ms,&app,&QApplication::quit);}
