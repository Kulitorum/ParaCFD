...
namespace paracfd::gui
{
	...
	bool ParagliderWindow::buildGrid()
	{
		...
		const std::size_t bricks=hierarchy.active_brick_count(),dofs=core->pressure_system().storage_size,gpu=core->gpu_bytes();const int muscl=core->embedded_high_order_stencil_count(),ls_full=core->embedded_least_squares_full_rank_count(),wall_nodes=core->fabric_wall_node_count();const QString debug_header=thin_debug?QString("CROPPED-Y DEBUG — %1 CELLS / %2 m\n").arg(debug_layers).arg(debug_width,0,'g',5):QString{};solver_readout_->setText(QString("%1PREPROCESS READY — STARTING CFD\n%2 bricks / %3 pressure DOFs\n%4 EB fragments / %5 apertures / %6 patches\n%7 complete compact MUSCL stencils\n%8 full-rank LS components / %9 fabric wall volumes\n%10 unresolved / %11 static pockets\n%12 sub-grid apertures discarded (%13 m²)\nGPU estimate %14 MiB").arg(debug_header).arg(bricks).arg(dofs).arg(fragments).arg(apertures).arg(patches).arg(muscl).arg(ls_full).arg(wall_nodes).arg(unresolved).arg(static_pockets).arg(discarded_apertures).arg(discarded_aperture_area,0,'g',4).arg(gpu/(1024.0*1024.0),0,'f',1));std::fprintf(stderr,"[paraglider] grid: %zu bricks, %zu DOFs, fragments=%zu apertures=%zu MUSCL=%d LS-full=%d wall-nodes=%d patches=%zu unresolved=%zu discarded_apertures=%zu area=%.6g m2, GPU %.2f MiB\n",bricks,dofs,fragments,apertures,muscl,ls_full,wall_nodes,patches,unresolved,discarded_apertures,discarded_aperture_area,gpu/(1024.0*1024.0));spawnWorker(std::move(core));play_button_->setEnabled(true);step_button_->setEnabled(true);{const QSignalBlocker blocker(play_button_);play_button_->setChecked(true);play_button_->setText("Pause");}worker_->setPlaying(true);viewer_->setSimulationState(true,false,auto_pause_->isChecked(),false,0,0);statusBar()->showMessage(thin_debug?QString("Cropped-Y diagnostic running: %1 cells / %2 m. Watch the STEP counter.").arg(debug_layers).arg(debug_width,0,'g',5):"CFD grid built; paraglider simulation running with freestream +X.",8000);return true;
	}
