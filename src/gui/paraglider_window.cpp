...
namespace paracfd::gui
{
	...
	void ParagliderWindow::buildControls()
	{
		...
		auto* run_group=new QGroupBox("Run");auto* run_column=new QVBoxLayout(run_group);build_button_=new QPushButton("Build CFD Grid + Run");build_button_->setToolTip("Build the static AMR/embedded-boundary grid, initialize the pressure field, and immediately run the CFD simulation.");play_button_=new QPushButton("Play");play_button_->setCheckable(true);step_button_=new QPushButton("Step");play_button_->setEnabled(false);step_button_->setEnabled(false);conservative_momentum_=new QCheckBox("Conservative momentum (experimental)");conservative_momentum_->setChecked(false);conservative_momentum_->setToolTip("Use the new control-volume momentum path on regular cells and split fluid fragments. It passes the current long-run validation but is not yet the trusted/default aerodynamic result path. Build Grid applies this choice.");auto_pause_=new QCheckBox("Auto-pause when settled");auto_pause_->setChecked(true);auto_pause_->setToolTip("Pause while preserving GPU state after force and velocity statistics remain stationary. The wake is observed for at least 1.5 domain flow-through times.");auto_pause_sensitivity_=new ScrollSafeSlider(Qt::Horizontal);auto_pause_sensitivity_->setRange(0,100);auto_pause_sensitivity_->setValue(65);auto_pause_sensitivity_->setToolTip("Left is cautious (requires a nearly steady field); right pauses earlier and tolerates more fluctuation.");connect(build_button_,&QPushButton::clicked,this,[this]{buildGrid();});connect(play_button_,&QPushButton::toggled,this,[this](bool on){play_button_->setText(on?"Pause":"Play");if(worker_)worker_->setPlaying(on);});connect(step_button_,&QPushButton::clicked,this,[this]{if(worker_)worker_->stepOnce();});connect(auto_pause_,&QCheckBox::toggled,this,[this](bool){if(worker_)worker_->configureAutoPause(auto_pause_->isChecked(),auto_pause_sensitivity_->value()/100.0);});connect(auto_pause_sensitivity_,&QSlider::valueChanged,this,[this](int){if(worker_)worker_->configureAutoPause(auto_pause_->isChecked(),auto_pause_sensitivity_->value()/100.0);});auto* run_row=new QHBoxLayout;run_row->addWidget(play_button_);run_row->addWidget(step_button_);auto* sensitivity_form=new QFormLayout;sensitivity_form->addRow("Settle sensitivity",auto_pause_sensitivity_);run_column->addWidget(build_button_);run_column->addLayout(run_row);run_column->addWidget(conservative_momentum_);run_column->addWidget(auto_pause_);run_column->addLayout(sensitivity_form);column->addWidget(run_group);
	...
	bool ParagliderWindow::buildGrid()
	{
		...
		const Vec3d size=hierarchy.domain().hi-hierarchy.domain().lo;SimInfo info;info.h=hierarchy.levels().front().h;info.coarse_h=info.h;info.finest_h=hierarchy.finest_cell_size();info.nx=(int)std::llround(size.x/info.h);info.ny=(int)std::llround(size.y/info.h);info.nz=(int)std::llround(size.z/info.h);info.Lx=size.x;info.Ly=size.y;info.Lz=size.z;info.U=run_config.freestream.speed;info.rho=run_config.freestream.rho;info.nu=run_config.freestream.nu;info.name=thin_debug?"thin-y-debug":"paraglider";viewer_->setInfo(info);slice_position_->setValue(500);
		...
		const std::size_t bricks=hierarchy.active_brick_count(),dofs=core->pressure_system().storage_size,gpu=core->gpu_bytes();const int muscl=core->embedded_high_order_stencil_count(),ls_full=core->embedded_least_squares_full_rank_count(),wall_nodes=core->fabric_wall_node_count();const QString mode=core->uses_conservative_cell_momentum()?"CONSERVATIVE CONTROL-VOLUME — EXPERIMENTAL":"STAGGERED MAC — DEFAULT";const QString debug_header=thin_debug?QString("CROPPED-Y DEBUG — %1 CELLS / %2 m\n").arg(debug_layers).arg(debug_width,0,'g',5):QString{};solver_readout_->setText(QString("%1%2\nPREPROCESS READY — STARTING CFD\n%3 bricks / %4 pressure DOFs\n%5 EB fragments / %6 apertures / %7 patches\n%8 complete compact MUSCL stencils\n%9 full-rank LS components / %10 fabric wall volumes\n%11 unresolved / %12 static pockets\n%13 sub-grid apertures discarded (%14 m²)\nGPU estimate %15 MiB").arg(debug_header).arg(mode).arg(bricks).arg(dofs).arg(fragments).arg(apertures).arg(patches).arg(muscl).arg(ls_full).arg(wall_nodes).arg(unresolved).arg(static_pockets).arg(discarded_apertures).arg(discarded_aperture_area,0,'g',4).arg(gpu/(1024.0*1024.0),0,'f',1));std::fprintf(stderr,"[paraglider] grid: mode=%s, %zu bricks, %zu DOFs, fragments=%zu apertures=%zu MUSCL=%d LS-full=%d wall-nodes=%d patches=%zu unresolved=%zu discarded_apertures=%zu area=%.6g m2, GPU %.2f MiB\n",core->uses_conservative_cell_momentum()?"conservative-cell":"staggered-default",bricks,dofs,fragments,apertures,muscl,ls_full,wall_nodes,patches,unresolved,discarded_apertures,discarded_aperture_area,gpu/(1024.0*1024.0));spawnWorker(std::move(core));play_button_->setEnabled(true);step_button_->setEnabled(true);{const QSignalBlocker blocker(play_button_);play_button_->setChecked(true);play_button_->setText("Pause");}worker_->setPlaying(true);viewer_->setSimulationState(true,false,auto_pause_->isChecked(),false,0,0);statusBar()->showMessage(thin_debug?QString("Cropped-Y diagnostic running: %1 cells / %2 m. Watch the STEP counter.").arg(debug_layers).arg(debug_width,0,'g',5):QString("CFD grid built; %1 running with freestream +X.").arg(mode),8000);return true;
	}
	...
	void ParagliderWindow::updateSnapshot()
	{
		...
		if(!s.error.empty())
		{
			solver_readout_->setText(QString("SOLVER STOPPED: %1").arg(QString::fromStdString(s.error)));QSignalBlocker blocker(play_button_);play_button_->setChecked(false);play_button_->setText("Play");viewer_->setSimulationState(false,false,s.auto_pause_enabled,false,0,s.flow_throughs);return;
		}
		{QSignalBlocker blocker(play_button_);play_button_->setChecked(s.playing);play_button_->setText(s.playing?"Pause":"Play");}
		viewer_->setSimulationState(s.playing,s.auto_paused,s.auto_pause_enabled,s.settling_ready,s.settling_score,s.flow_throughs);
		if(!s.initialized){solver_readout_->setText("Initializing pressure field on GPU...");return;}
		last_steps_=s.steps;last_time_=s.physical_time;viewer_->setSimulationStep(s.steps);
