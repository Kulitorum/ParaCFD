...
namespace paracfd::gui
{
	...
	bool ParagliderWindow::buildGrid()
	{
		...
		if(thin_debug){viewer_->setMesh(wing);viewer_->setMeshPlacement(ModelPlacement{});thin_debug_display_=true;half_wing_display_=false;slice_axis_->setCurrentIndex(1);slice_position_->setValue(500);arrow_mode_->setCurrentIndex(1);tracer_mode_->setCurrentIndex(1);viewer_->setThinDebugState(true,debug_layers);viewer_->setSimulationStep(0);viewer_->frameThinYDebugView();}else if(half_wing){viewer_->setMesh(wing);viewer_->setMeshPlacement(ModelPlacement{});thin_debug_display_=false;half_wing_display_=true;viewer_->setThinDebugState(false,0);viewer_->setSimulationStep(0);viewer_->frameWingView();}else{thin_debug_display_=false;half_wing_display_=false;viewer_->setMeshPlacement(config_.placement);viewer_->setThinDebugState(false,0);viewer_->frameWingView();}viewer_->setReferenceU(info.U);updateDebugBoxes();
