...
namespace paracfd::gui
{
	...
	void ParagliderWindow::buildControls()
	{
		...
		auto* thin_group=new QGroupBox("Thin-slice diagnosis");auto* thin_form=new QFormLayout(thin_group);thin_y_debug_=new QCheckBox("Run only 2 Y cells");thin_y_debug_->setToolTip("Crop the oriented wing to a two-cell-thick Y slab and run a uniform X-Z-dominant case at the configured finest spacing. This is a diagnostic solver case, not a physical whole-wing result.");thin_y_fraction_=real_spin(0.05,0.95,0.5,3);thin_y_fraction_->setSingleStep(0.025);thin_y_fraction_->setToolTip("Span station through the oriented wing bbox: 0 is one tip, 0.5 is centre, 1 is the other tip.");thin_form->addRow(thin_y_debug_);thin_form->addRow("Span station",thin_y_fraction_);column->addWidget(thin_group);connect(thin_y_debug_,&QCheckBox::toggled,this,[this]{updateGridReadout();});connect(thin_y_fraction_,QOverload<double>::of(&QDoubleSpinBox::valueChanged),this,[this]{updateGridReadout();});
	...
	void ParagliderWindow::setThinYDiagnostic(double span_fraction)
	{
		if(!thin_y_debug_||!thin_y_fraction_)return;thin_y_fraction_->setValue(std::clamp(span_fraction,0.05,0.95));thin_y_debug_->setChecked(true);
	}
	...
	{
		if(!grid_readout_)return;const auto c=configFromUi();const int ratio=1<<std::max(0,c.amr.max_levels-1);const double finest=c.amr.base_cell_size/ratio;QString text=QString("finest h = %1 m (2:1 ×%2)\nbrick = %3³, coarse width %4 m\nEB merge < %5 h³, aperture cutoff < %6 h²\nproduction fields = %7").arg(finest,0,'g',5).arg(ratio).arg(c.amr.brick_size).arg(c.amr.base_cell_size*c.amr.brick_size,0,'g',5).arg(c.amr.min_volume_fraction,0,'g',4).arg(c.amr.min_aperture_area_fraction,0,'g',4).arg(sizeof(Real)==4?"FP32":"FP64 validation");if(thin_y_debug_&&thin_y_debug_->isChecked())text+=QString("\nTHIN-Y DEBUG: uniform h=%1 m, 2 cells (%2 m), station=%3").arg(finest,0,'g',5).arg(2*finest,0,'g',5).arg(thin_y_fraction_->value(),0,'f',3);grid_readout_->setText(text);
	}
	...
	bool ParagliderWindow::buildGrid()
	{
		...
		const bool thin_debug=thin_y_debug_&&thin_y_debug_->isChecked();ParagliderConfig run_config=config_;TriMesh wing;
		if(thin_debug)
		{
			const int ratio=1<<std::max(0,config_.amr.max_levels-1);const double h=config_.amr.base_cell_size/ratio;constexpr int layers=2;const double thickness=layers*h;ModelPlacement orientation=config_.placement;orientation.tx=orientation.ty=orientation.tz=0;const TriMesh oriented=placed_mesh(source_mesh_,orientation);const double fraction=thin_y_fraction_->value(),centre=oriented.bbox_min[1]+fraction*(oriented.bbox_max[1]-oriented.bbox_min[1]);TriMesh clipped=clip_mesh_to_axis_slab(oriented,1,centre-0.5*thickness,centre+0.5*thickness);if(clipped.empty()){QMessageBox::critical(this,"Thin slice failed","The selected Y slab contains no fabric triangles.");return false;}run_config.amr.base_cell_size=h;run_config.amr.max_levels=1;run_config.amr.brick_size=layers;run_config.domain.lateral_margin=0;run_config.reference.area=0;run_config.reference.length=0;const ModelPlacement frame=frame_wing_for_external_domain(clipped,ModelPlacement{},run_config.domain.upstream_margin,0,run_config.domain.vertical_margin,h*layers);wing=placed_mesh(clipped,frame);run_config.placement=ModelPlacement{};std::fprintf(stderr,"[paraglider-thin-y] station=%.6g source-y=%.6g thickness=%.6g m h=%.6g layers=%d triangles=%zu\n",fraction,centre,thickness,h,layers,wing.triangle_count());
		}
		...
		if(thin_debug){viewer_->setMesh(wing);viewer_->setMeshPlacement(ModelPlacement{});thin_debug_display_=true;slice_axis_->setCurrentIndex(1);slice_position_->setValue(500);arrow_mode_->setCurrentIndex(1);tracer_mode_->setCurrentIndex(1);viewer_->setThinDebugState(true,2);viewer_->setSimulationStep(0);viewer_->frameThinYDebugView();}else{viewer_->setMeshPlacement(config_.placement);viewer_->setThinDebugState(false,0);viewer_->frameWingView();}viewer_->setReferenceU(info.U);updateDebugBoxes();
		const std::size_t bricks=hierarchy.active_brick_count(),dofs=core->pressure_system().storage_size,gpu=core->gpu_bytes();const int muscl=core->embedded_high_order_stencil_count();solver_readout_->setText(QString("%1PREPROCESS READY — STARTING CFD\n%2 bricks / %3 pressure DOFs\n%4 EB fragments / %5 apertures / %6 patches\n%7 complete compact MUSCL stencils\n%8 unresolved / %9 static pockets\n%10 sub-grid apertures discarded (%11 m²)\nGPU estimate %12 MiB").arg(thin_debug?"THIN-Y DEBUG — 2 CELLS\n":"").arg(bricks).arg(dofs).arg(fragments).arg(apertures).arg(patches).arg(muscl).arg(unresolved).arg(static_pockets).arg(discarded_apertures).arg(discarded_aperture_area,0,'g',4).arg(gpu/(1024.0*1024.0),0,'f',1));std::fprintf(stderr,"[paraglider] grid: %zu bricks, %zu DOFs, fragments=%zu apertures=%zu MUSCL=%d patches=%zu unresolved=%zu discarded_apertures=%zu area=%.6g m2, GPU %.2f MiB\n",bricks,dofs,fragments,apertures,muscl,patches,unresolved,discarded_apertures,discarded_aperture_area,gpu/(1024.0*1024.0));spawnWorker(std::move(core));play_button_->setEnabled(true);step_button_->setEnabled(true);if(!play_button_->isChecked())play_button_->setChecked(true);else worker_->setPlaying(true);statusBar()->showMessage(thin_debug?"Thin-Y diagnostic running: watch the prominent STEP counter for field collapse.":"CFD grid built; paraglider simulation running with freestream +X.",8000);return true;
	}
