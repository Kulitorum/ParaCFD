...
namespace paracfd::gui
{
	...
	{
		config_=c;speed_->setValue(c.freestream.speed);rho_->setValue(c.freestream.rho);nu_->setValue(c.freestream.nu);tessellation_->setValue(c.tessellation_deflection_mm);upstream_->setValue(c.domain.upstream_margin);downstream_->setValue(c.domain.downstream_margin);lateral_->setValue(c.domain.lateral_margin);vertical_->setValue(c.domain.vertical_margin);base_h_->setValue(c.amr.base_cell_size);levels_->setValue(c.amr.max_levels);brick_size_->setValue(c.amr.brick_size);wing_refine_->setValue(c.amr.wing_refinement_distance);surface_refine_->setValue(c.amr.surface_refinement_distance);wake_length_->setValue(c.amr.wake_length);wake_radius_->setValue(c.amr.wake_radius);cfl_->setValue(c.solver.cfl);smagorinsky_->setValue(c.solver.smagorinsky_cs);projection_tolerance_->setValue(c.solver.projection_tolerance);projection_iterations_->setValue(c.solver.projection_max_iterations);reference_area_->setValue(c.reference.area);reference_length_->setValue(c.reference.length);viewer_->setReferenceU(c.freestream.speed);updateGridReadout();
	}

	ParagliderConfig ParagliderWindow::configFromUi()const
	{
		ParagliderConfig c=config_;c.step_path=step_path_.toStdString();c.placement=config_.placement;c.tessellation_deflection_mm=tessellation_->value();c.freestream={speed_->value(),rho_->value(),nu_->value()};c.domain={upstream_->value(),downstream_->value(),lateral_->value(),vertical_->value()};c.amr.base_cell_size=base_h_->value();c.amr.max_levels=levels_->value();c.amr.brick_size=brick_size_->value();c.amr.wing_refinement_distance=wing_refine_->value();c.amr.surface_refinement_distance=surface_refine_->value();c.amr.wake_length=wake_length_->value();c.amr.wake_radius=wake_radius_->value();c.solver.cfl=cfl_->value();c.solver.smagorinsky_cs=smagorinsky_->value();c.solver.projection_tolerance=projection_tolerance_->value();c.solver.projection_max_iterations=projection_iterations_->value();c.reference.area=reference_area_->value();c.reference.length=reference_length_->value();return c;
	}
