...
; CMake Release build output (windcfd-gui.exe + the full deployed DLL/plugin closure).
; Build first:  cmake --build build --config Release --target windcfd-gui
...
; re-runs when windcfd-gui relinks, so build/Release can silently lack them — freetype.dll is a
