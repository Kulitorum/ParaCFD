...
; CMake Release build output (scour-gui.exe + the full deployed DLL/plugin closure).
; Build first:  cmake --build build --config Release --target scour-gui
...
; re-runs when scour-gui relinks, so build/Release can silently lack them — freetype.dll is a
