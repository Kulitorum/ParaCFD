...
; --- The CMake Release build output (holds paracfd-gui.exe + the Qt/OpenCascade/3rdparty DLL
;     closure deployed by windeployqt + paracfd_deploy_occ_dlls, and the Qt plugin subdirs) ---
;     Build it first:  cmake --build build --config Release --target paracfd-gui
...
;     (recording then needs ffmpeg on PATH or the PARACFD_FFMPEG env var at run time). ----------
