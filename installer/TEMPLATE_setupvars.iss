...
#define ProjectDir "C:\CODE\ScourProtection"

; --- The CMake Release build output (holds scour-gui.exe + the Qt/OpenCascade/3rdparty DLL
;     closure deployed by windeployqt + scour_deploy_occ_dlls, and the Qt plugin subdirs) ---
;     Build it first:  cmake --build build --config Release --target scour-gui
#define AppBuildDir "C:\CODE\ScourProtection\build\Release"
...
;     (recording then needs ffmpeg on PATH or the SCOUR_FFMPEG env var at run time). ----------
