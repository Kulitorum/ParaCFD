# ScourProtection installer (Inno Setup, signed)

Builds a signed Windows installer that packs `scour-gui.exe` with its **entire** runtime closure
so it runs on a clean machine (no Qt / OpenCascade / dev tools required — only an NVIDIA driver).

## What it packs
- **App**: `scour-gui.exe` (signed).
...
- **OpenCascade 8.0** STEP toolkits (`TK*.dll`) and their 3rdparty DLLs
  (`freetype`, `brotli*`, `bz2`, `libpng16`, `zlib1`) — deployed by `scour_deploy_occ_dlls`.
...
installed `scour-gui.exe`, the uninstaller (`SignedUninstaller`) and the generated setup `.exe` are
...
.\installer\build_installer.ps1 -Build     # rebuild scour-gui first, then compile
.\installer\build_installer.ps1 -NoSign    # UNSIGNED (validate packaging without the token)
```
Or open `ScourProtection-installer.iss` in the Inno Setup IDE and press *Compile*.

## Files
- `ScourProtection-installer.iss` — the installer script.
