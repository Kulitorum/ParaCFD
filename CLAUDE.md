...
Independent EB aperture-normal velocities are advanced on a compact same-side graph. Structured MAC faces adjacent to each fragment supply the carrier state, and each aperture uses a bounded first-order upwind update plus molecular diffusion and a graph-normal Smagorinsky estimate before projection. The combined explicit update is limited to the extrema of its actual same-side stencil, so transport cannot invent a new local maximum or minimum; there is no arbitrary absolute velocity cap, and the subsequent pressure projection remains free to create physically required extrema. The graph contains only actual fluid connections, so it cannot transport through fabric and it does not turn the regular domain into CSR. This first implementation still omits a full irregular strain tensor, a wall model, and higher-order reconstruction on the compact graph.
...
- The new external-aero timestep is wired end to end with bounded MacCormack/RK2 advection. Fabric-near faces use a precomputed same-side link graph with minmod-limited linear transport where the full connected stencil exists, a first-order fallback at incomplete stencils, and link-restricted molecular diffusion. Same-level brick crossings and normal 2:1 interface fluxes are synchronized conservatively; general cross-level interpolation remains nonconservative.
- Compact EB aperture velocities use a bounded first-order same-side graph transport with molecular diffusion plus a graph-normal Smagorinsky estimate. Conservative 0.25-volume merging and the reportable `1e-4 h^2` aperture cutoff remove numerical slivers. The LES term preserves uniform flow, dissipates manufactured nonuniform compact states, and removes the refinement-driven aft-junction growth; the combined explicit update cannot leave its local same-side stencil range. This is not a full irregular strain tensor or wall model, and higher-order reconstruction is still required before trusting loads.
- With the confirmed PlanB leading edge facing upstream, graph LES, and limited fabric-band transport, a canonical-frame three-level 500-step diagnostic reaches 0.531 s with an 11.99 m/s regular peak, a 46.8 m/s compact-graph peak, and pressure-only force `[145.9, 0.10, 10.0]` N. The background mean remains `[10.009, -1.3e-5, -2.5e-4]` m/s with no reverse-flow volume; volume-weighted RMS divergence is `3.93e-6 s^-1` and net integrated flux error is `-3.15e-5 m^3/s`. The compact peak remains too high for trustworthy loads and is an explicit momentum-discretization/convergence blocker, not something hidden by a velocity clamp. Equal-time pressure loads are still grid-dependent, so these are stability/conservation diagnostics, not aerodynamic convergence. Higher-order compact-EB transport, full grid/force convergence, and AoA confirmation remain necessary.
- The Qt viewer displays live velocity/pressure slices, arrows, fabric-stopped tracers, AMR/EB boxes, and triangle-native visible-side Cp, Cp+, Cp-, or delta-Cp. The scalar slice is finest-brick aware, but visualization still uses throttled host snapshots and OpenGL uploads rather than direct CUDA/OpenGL interop; arrows and tracers use a coarse uniform 3-D display copy.
