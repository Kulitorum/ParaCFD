...
- The new external-aero timestep is wired end to end with bounded MacCormack/RK2 advection. Fabric-near faces use a conservative precomputed same-side link graph with first-order donor transport and molecular diffusion. Same-level brick crossings and normal 2:1 interface fluxes are synchronized conservatively; general cross-level interpolation remains nonconservative.
...
- With graph LES, a canonical-frame three-level 500-step PlanB diagnostic reaches 0.677 s with a 39.2 m/s compact peak and pressure-only force `[131.9, -0.52, 39.4]` N. At four levels, 500 steps reach 0.294 s with a 48.7 m/s compact peak instead of the previous 78.8 m/s at 0.202 s (and 102 m/s by 0.457 s). The regular peaks remain 13.1 and 14.7 m/s. Equal-time pressure loads are still roughly 12-14% apart, so these are stability/conservation diagnostics, not aerodynamic convergence. Grid/force convergence, higher-order EB transport, and orientation/AoA confirmation remain necessary.
