...
Every surface patch also contributes to a separate compact two-sided fabric-wall work list, including a sheet exactly coincident with a Cartesian face and therefore having no compact aperture edge. Each side uses its own fluid-control-volume centroid and pressure DOF. The stationary no-slip boundary contributes the actual patch-area/normal-distance finite-volume molecular viscous coefficient `nu A/(V d_n)`. A local backward-Euler sink damps the reconstructed wall-side velocity, then distributes that change to its incident regular carriers and compact apertures; the two fabric sides never share a state. The treatment is invariant to triangle winding and unconditionally stable for small wall distances. This is a first resolved molecular no-slip treatment, not a high-Reynolds-number fabric wall function: wall-modelled turbulent shear and skin-friction load accumulation remain missing.
...
Every owned composite surface patch retains its global plus/minus pressure DOFs and CAD triangle/face provenance. Surface pressure results retain `p_plus`, `p_minus`, `delta_p`, the corresponding Cp values, and pressure force per source triangle. Whole-wing force and moment use double-precision accumulation. With freestream along +X, drag, side, and lift axes are +X, +Y, and +Z. Coefficients are reported only when the user supplies a positive reference area; the present aerodynamic load is explicitly pressure-only because viscous wall traction is not yet accumulated back to surface patches.
...
- A separate one-level finite-volume MAC kernel is the conservative-momentum foundation. It evaluates each upwind momentum flux symmetrically at both endpoints, preserves constant vector fields, conserves the discrete lattice momentum across same-level brick boundaries, and suppresses links cut by fabric. It is not called by `ExternalAeroCore` yet: compact EB aperture exchange and staggered 2:1 flux registers must join the same update before replacing production advection.
- Compact EB aperture velocities use bounded minmod-limited MUSCL transport on complete same-axis graph chains, with a first-order donor fallback where a chain ends or branches, plus molecular diffusion and component-collocated weighted least-squares reconstruction of the full velocity-gradient tensor. Conservative 0.25-volume merging and the reportable `1e-4 h^2` aperture cutoff remove numerical slivers. Tests cover a smooth quadratic characteristic, uniform-state preservation, local bounds, LES dissipation, tangential shear, rigid rotation, normal extension, exact affine reconstruction, winding-invariant inclined no-slip damping, and wall flux on a face-aligned membrane with no compact apertures. A high-Re wall function and force convergence are still required before trusting loads.
- On the current 47,997-triangle PlanB working fixture with the confirmed leading edge facing upstream, the conditioned least-squares/no-slip build has 78,299 full-rank fragment/component stencils and 23,105 wall-side control volumes. A 1,500-step diagnostic reaches 3.316 s with 12.33/17.31 m/s regular/compact peaks and pressure-only force `[167.62, -3.91, -32.58]` N. Volume-weighted RMS divergence is `5.17e-6 s^-1`, net integrated flux error is `-6.10e-5 m^3/s`, the final step is 62.3 ms including 20.3 ms projection and 0.29 ms compact/wall transport, and persistent storage is 509.32 MiB. The run stays stable beyond the former roughly 1,000-step collapse window without an absolute velocity guard. Loads remain transient and grid-dependent; full grid/domain/force convergence and AoA confirmation remain necessary.
...
- Molecular no-slip wall flux affects the flow, but wall-modelled turbulent shear and viscous traction accumulation are absent; reported force is pressure-only.
