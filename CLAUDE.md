...
- Compact EB aperture velocities use a first-order same-side graph transport with molecular diffusion. Conservative 0.25-volume merging and the reportable `1e-4 h^2` aperture cutoff remove numerical slivers, but they do not establish grid convergence. A four-level PlanB run exposes a resolution-dependent compact-aperture hotspot, so higher-order reconstruction, consistent Smagorinsky treatment, and a local velocity/flux acceptance gate are required before trusting loads.
- A canonical-frame three-level 500-step PlanB diagnostic reaches 0.622 s. The former structured leading-edge runaway is removed: the regular peak stays near 12.1 m/s for a 10 m/s inlet. A compact EB aperture state still peaks near 46.0 m/s, and the pressure-only force remains evolving near `[132.0, -0.57, 38.1]` N. At four levels, 500 steps reach only 0.202 s and the compact peak grows to 78.8 m/s while the regular peak remains 13.9 m/s. These are solver diagnostics at unequal physical times, not an aerodynamic convergence comparison. Grid/force convergence, higher-order EB transport, and orientation/AoA confirmation remain necessary.
