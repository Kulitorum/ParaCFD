...
- a separately callable one-level finite-volume MAC advection foundation whose shared upwind link fluxes conserve discrete momentum across same-level bricks, preserve constants, and cannot cross fabric, plus a CPU/GPU pairwise SoA primitive for equal-and-opposite vector-momentum transfer; static 2:1 preprocessing assigns exact half-coarse/half-fine normal-tile volumes and computes tangential staggered-dual overlaps with canonical same-level ownership; a persistent compact GPU map gathers/scatters those canonical addresses directly against pooled level fields, and manufactured gates cover exact tiling/reflux, map round trips, constants, fabric-edge omission, and CUDA parity; replaced-link suppression and compact EB coupling are not wired yet;
...
1. Compose the validated GPU MAC gather/scatter map with the normal-tile/tangential-overlap pairwise update, suppress the replaced structured links, and couple compact EB states; switch `ExternalAeroCore` only after composite constant/momentum gates pass, then rerun the equal-time convergence gate.
