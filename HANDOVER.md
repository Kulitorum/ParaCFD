...
- a separately callable one-level finite-volume MAC advection foundation whose shared upwind link fluxes conserve discrete momentum across same-level bricks, preserve constants, and cannot cross fabric, plus CPU/GPU pairwise SoA primitives for equal-and-opposite component momentum transfer; static 2:1 preprocessing assigns exact normal-tile volumes and tangential staggered-dual overlaps, a persistent GPU map bridges canonical addresses to pooled fields, and a 1,462-node/1,702-link gate conserves mapped momentum to `8.9e-15` with `1.85e-7` FP32 (`2.22e-16` FP64) CUDA error; 7,920 unique structured directions are statically cleared where compact interface fluxes replace them, without changing physical boundary links; compact EB coupling is not wired yet;
