...
- about 507.6 MiB total persistent estimate for fields, projection, compact EB transport with directed chain links plus least-squares matrices/RHS, two advection states/masks, and locator.

`paraglider_case_probe` records long imported-wing histories in the exact frame used by the GUI. On the current user-owned 47,997-triangle PlanB working fixture, with the confirmed leading edge facing upstream, default 0.25 same-side fragment merge, `1e-4 h^2` aperture cutoff, bounded fabric-band/MUSCL compact transport, component-collocated least-squares graph strain, adaptive CFL, and 1e-5 projection tolerance, 78,299 fragment/component stencils are well-conditioned and full rank. A 1,500-step run reaches 3.175 s. Regular/compact peaks are 12.36/18.00 m/s and the still-transient pressure-only force is `[167.70, -3.72, -32.73]` N. Max/RMS-volume divergence are `6.42e-4 / 4.90e-6 s^-1`, and net integrated flux error is `-4.78e-5 m^3/s`. The last measured step is 60.2 ms, including 18.0 ms projection and 0.30 ms compact EB transport; individual pressure iteration counts and timings vary. Persistent storage is 507.61 MiB. The run remains stable beyond the former roughly 1,000-step collapse window without an absolute velocity guard. This is a stability/conservation result, not a converged aerodynamic result. `Test-Data/PlanBParakite.step` contains user working-tree edits and is intentionally not included in solver commits.
...
1. Add an arbitrary-orientation fabric wall treatment and validate no-slip/no-penetration on inclined sheets.
