...
- a separately callable one-level finite-volume MAC advection foundation plus CPU/GPU pairwise SoA component transport; regular face volume comes from half of each active adjacent pressure cell, normal tiles replace the covered-side share exactly, and tangential links use clipped staggered-dual overlaps; a persistent map bridges canonical faces to pooled fields, the 1,462-node/1,702-link tangential gate conserves to `3.55e-15` with `1.67e-7` FP32 (zero FP64) error, and the 9-node/8-link normal gate conserves to `3.47e-18` while scattering one four-tile coarse mean within `3.03e-8`; 7,920 compact-owned structured directions are cleared; compact EB coupling is not wired yet;
