...
- a separate compact two-sided fabric-wall work list sourced from every surface patch, including face-aligned sheets with no compact aperture; production applies an implicit molecular no-slip flux using actual patch area, volume, and normal centroid distance, then updates incident regular carriers/compact apertures without connecting the two sides; a separately callable GPU Spalding smooth-wall path uses arbitrary patch orientation and actual wall distance, passes low/high-Re, dissipation, and winding tests in FP32/FP64, and remains staged until viscous traction is accumulated;
...
2. Accumulate the staged Spalding wall flux as equal-and-opposite viscous CAD-triangle traction, then promote it to production separately from pressure loads.
