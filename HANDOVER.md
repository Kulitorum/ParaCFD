...
- the earlier compact carrier-ring transport remains a negative/reference experiment: it conserves its isolated subset but has no free-stream-preserving mortar to the surrounding staggered dual volumes. Its replacement stores vector momentum directly on each actual pressure control volume (regular cell or split fragment), with projected MAC/aperture velocities supplying mass flux. On one uniform level, regular faces stay implicit in a brick CUDA kernel and only the 110 perimeter plus 96 EB connections in the membrane fixture are compact. Freestream is exact; perturbed CPU/GPU state error is `4.66e-10` FP32 / zero FP64, integrated conservation is `2.68e-7` FP32 storage rounding / `2.13e-14` FP64, and external transverse export is analytically exact. It is not called by `ExternalAeroCore` until 2:1, reconstruction, and diffusion are complete;
...
1. Extend the new cell/fragment-centred momentum state across conservative 2:1 flux tiles, reconstruct it from corrected face fluxes, and add same-side LES/diffusion; switch `ExternalAeroCore` only after multilevel constant/momentum/boundary gates pass, then rerun the equal-time convergence gate.
