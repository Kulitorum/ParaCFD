...
1. Reconstruct the new multilevel cell/fragment-centred momentum state from corrected face fluxes and add same-side LES/diffusion; switch `ExternalAeroCore` only after the composed constant/momentum/boundary gates pass, then rerun the equal-time convergence gate.
