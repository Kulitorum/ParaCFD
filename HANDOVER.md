...
- a separately callable one-level finite-volume MAC advection foundation whose shared upwind link fluxes conserve discrete momentum across same-level bricks, preserve constants, and cannot cross fabric, plus a CPU/GPU pairwise SoA primitive for equal-and-opposite vector-momentum transfer; static 2:1 preprocessing assigns exact half-coarse/half-fine normal-tile volumes and computes tangential staggered-dual overlaps with canonical same-level ownership, while manufactured gates cover exact tiling/reflux, constants, fabric-edge omission, and CUDA parity; production gather/scatter and compact EB coupling are not wired yet;
...
1. Upload the validated normal-tile/tangential-overlap 2:1 maps, suppress the replaced structured links, and couple them with compact EB states in the pairwise conservative update; switch `ExternalAeroCore` only after composite constant/momentum gates pass, then rerun the equal-time convergence gate.
