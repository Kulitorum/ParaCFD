...
1. Add same-side LES to the new multilevel cell/fragment-centred momentum state and exercise transport -> diffusion/LES -> reconstruction -> projection -> direct pressure impulse as one repeated timestep; switch `ExternalAeroCore` only after its constant-state, boundary, EB, AMR, and long-run gates pass, then rerun equal-time convergence.
