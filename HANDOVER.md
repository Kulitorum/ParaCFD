...
- a separately callable one-level finite-volume MAC advection foundation plus CPU/GPU pairwise SoA component transport; regular face volume comes from half of each active adjacent pressure cell, normal tiles replace the covered-side share exactly, and tangential links use clipped staggered-dual overlaps; one unified graph owns 7,023 canonical faces and 7,840 transfers, including 501 multi-axis refinement-corner states, conserving each component to `1.47e-14` and matching CUDA within `2.95e-8` FP32 (zero FP64); coarse normal faces remain area-mean aliases, 7,920 compact-owned structured directions are cleared, and compact EB coupling is not wired yet;
