...
- a separately callable one-level finite-volume MAC advection foundation whose shared upwind link fluxes conserve discrete momentum across same-level bricks, preserve constants, and cannot cross fabric, plus CPU/GPU pairwise SoA primitives for equal-and-opposite component momentum transfer; static 2:1 preprocessing assigns exact half-coarse/half-fine normal-tile volumes and tangential staggered-dual overlaps with canonical ownership; a persistent GPU map gathers/scatters those addresses against pooled fields, and a 1,462-node/1,702-link planar gate conserves mapped volume-weighted momentum to `8.9e-15` with `1.85e-7` FP32 (`2.22e-16` FP64) CUDA state error; replaced-link suppression and compact EB coupling are not wired yet;
