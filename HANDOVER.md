...
1. Resolve the FP32 pressure residual floor exposed by the second small correction in the repeated nonuniform wall/projection gate; verify true post-projection divergence without accepting a loose relative residual.
2. Add nonuniform external-boundary and long imported-wing repeated-timestep gates, then switch `ExternalAeroCore` to the conservative state and rerun equal-time PlanB grid/domain/force convergence.
