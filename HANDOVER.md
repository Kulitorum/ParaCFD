...
- a separately callable one-level finite-volume MAC advection foundation whose shared upwind link fluxes conserve discrete momentum across same-level bricks, preserve constants, and cannot cross fabric, plus a CPU/GPU pairwise SoA primitive that gives regular dual volumes, 2:1 tiles, and EB states one equal-and-opposite vector-momentum transfer contract; manufactured gates cover exact four-tile reflux, constants, fabric-edge omission, and CUDA parity, but production ownership maps are not coupled yet;
...
1. Build static pooled-MAC/compact-state ownership maps around the validated pairwise conservative primitive, including unique same-level links and four-tile 2:1 registers, then switch `ExternalAeroCore` only after the composite manufactured gates pass and rerun the equal-time convergence gate.
