...
- a selectable thin-Y collapse diagnostic that clips open fabric at a span station, runs the real solver on exactly two uniform Y cells at configured finest `h`, switches the viewer to a Y-normal 2-D presentation, and overlays an unmissable timestep counter. EB atlas halos are clamped at physical boundaries so stabilization cannot merge into a phantom halo cell.
...
- about 497.0 MiB total persistent estimate for fields, projection, compact EB transport with directed chain links plus node gradient sums/weights, two advection states/masks, and locator.

`paraglider_case_probe` records long imported-wing histories in the exact frame used by the GUI. On the current user-owned 47,997-triangle PlanB working fixture, with the confirmed leading edge facing upstream, default 0.25 same-side fragment merge, `1e-4 h^2` aperture cutoff, bounded fabric-band transport, MUSCL compact transport, reconstructed graph-strain Smagorinsky treatment, adaptive CFL, and 1e-5 projection tolerance, a 500-step run reaches 0.527 s. Regular/compact peaks are 12.76/45.45 m/s and the still-evolving pressure-only force is `[174.66, 0.02, -5.27]` N. Max/RMS-volume divergence are `1.20e-3 / 4.18e-6 s^-1`, and net integrated flux error is `3.89e-6 m^3/s`. The last measured step is 55.8 ms, including 14.2 ms projection and 0.28 ms compact EB transport; individual pressure iteration counts and timings vary. Persistent storage is 496.98 MiB. The immediately preceding MUSCL build with only graph-normal LES reached a 55.05 m/s compact peak at 500 steps, so reconstructed strain lowers it by about 17%. This remains a stability/conservation comparison, not a converged aerodynamic result; the compact peak and loads remain explicit blockers rather than being masked by a velocity guard. `Test-Data/PlanBParakite.step` contains user working-tree edits and is intentionally not included in solver commits.
...
Use `paraglider_case_probe --physical-time T --max-levels N` for new comparisons. It stops after crossing the requested physical time, emits a machine-readable `[paraglider-case-summary]`, and treats an explicitly supplied `--steps` as a safety ceiling. `--thin-y-fraction 0.5` reproduces the GUI's two-cell centre-span diagnostic. That PlanB diagnostic ran 100 steps to 0.290 s with regular/compact peaks 12.37/15.02 m/s, maximum divergence `1.98e-4 s^-1`, and about 25.1 MiB persistent storage; use the UI step overlay to correlate this numerical history with any visual field collapse. At `t~=0.100 s` on the current working fixture, 2/3/4 levels (finest 0.125/0.0625/0.03125 m) give pressure-only `[Fx,Fz]` of `[180.2,-8.17]`, `[200.6,-5.25]`, and `[203.3,-24.77]` N. The 3-to-4-level streamwise change is only 1.4%, but the vertical component is not converged and this early time is still transient. Do not use the apparent Fx agreement as a force-validation claim.
