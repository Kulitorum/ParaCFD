...
int main(int argc,char**argv)
{
	...
	std::size_t fragments=0,patches=0,apertures=0,unresolved=0,eb_bricks=0,static_pockets=0;std::map<std::string,std::size_t> unresolved_reasons;std::map<std::string,std::array<std::size_t,2>> unresolved_face_multiplicity;
	EmbeddedBoundaryBuildOptions eo;eo.min_volume_fraction=cfg.amr.min_volume_fraction;eo.complex_subdivisions=cfg.amr.complex_subdivisions;if(sheet_angle_degrees>0)eo.smooth_sheet_angle_tolerance=sheet_angle_degrees*3.14159265358979323846/180.0;if(sheet_distance_fraction>0)eo.smooth_sheet_distance_fraction=sheet_distance_fraction;if(complex_subdivisions>0)eo.complex_subdivisions=complex_subdivisions;
	...
	if(composite_ready)std::printf("  composite pressure topology: %zu DOFs, %zu coarse/fine + %zu EB connections (CPU/CUDA matrix-free operator ready)\n",composite_dofs,coarse_fine_connections,embedded_connections);else if(!unresolved)std::fprintf(stderr,"[paraglider] composite pressure topology rejected: %s\n",composite_error.c_str());
	std::printf("  limitation: velocity divergence/gradient and the complete external-aero timestep are not wired to this composite topology yet.\n");
	return unresolved?3:(composite_ready?0:4);
}
