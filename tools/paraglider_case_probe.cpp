...
int main(int argc,char** argv)
{
	...
	try
	{
		...
		std::printf("[paraglider-case] triangles=%zu bricks=%zu DOFs=%d EB-edges=%zu MUSCL=%d patches=%zu gauges=%zu discarded-apertures=%zu/%.6g-m2 load=%.2f ms preprocess=%.2f ms GPU=%.2f MiB tolerance=%.3e\n",wing.triangle_count(),core.hierarchy().active_brick_count(),core.pressure_system().storage_size,core.pressure_system().embedded.size(),core.embedded_high_order_stencil_count(),core.pressure_system().surface_patches.size(),core.pressure_system().gauges.size(),discarded_apertures,discarded_area,elapsed_ms(load_begin,load_end),elapsed_ms(build_begin,build_end),core.gpu_bytes()/(1024.0*1024.0),config.solver.projection_tolerance);
