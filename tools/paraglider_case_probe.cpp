...
int main(int argc,char** argv)
{
	...
	std::string config_path="configs/planb_parakite.json";int steps=50,sample_every=5,max_levels=0;double projection_tolerance=0,min_volume_fraction=0,smagorinsky_cs=-1,target_physical_time=0,thin_y_fraction=-1,thin_y_width=0.125;bool locate_regular_max=false,conservative_cell_momentum=false,no_smooth_wall=false,no_pressure_impulse=false,steps_explicit=false,thin_y_width_explicit=false;
	for(int i=1;i<argc;++i)
	{
		...
		else{std::fprintf(stderr,"usage: paraglider_case_probe [--config file] [--steps N] [--physical-time seconds] [--sample-every N] [--projection-tolerance value] [--min-volume-fraction value] [--smagorinsky-cs value] [--max-levels N] [--thin-y-fraction 0..1] [--thin-y-width metres] [--locate-regular-max] [--conservative-cell-momentum] [--no-smooth-wall] [--no-pressure-impulse]\n");return 2;}
	}
	...
	ParagliderConfig config;std::string error;if(!load_paraglider_config(config_path,config,&error)){std::fprintf(stderr,"[paraglider-case] %s\n",error.c_str());return 2;}if(projection_tolerance>0)config.solver.projection_tolerance=projection_tolerance;if(min_volume_fraction>0)config.amr.min_volume_fraction=min_volume_fraction;if(smagorinsky_cs>=0)config.solver.smagorinsky_cs=smagorinsky_cs;if(max_levels>0)config.amr.max_levels=max_levels;
