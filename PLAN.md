...
The imported-wing promotion gate is now connected behind `ExternalAeroExecutionOptions::conservative_cell_momentum`. Its projection-to-cell feedback uses the aperture-area-weighted exact face-normal correction from the composite projection, not the historical raw pressure-traction impulse described above; this keeps persistent momentum compatible with projected MAC/aperture flux on skewed merged fragments. Exact donor-rate CFL includes structured, EB, coarse/fine, and physical-boundary fluxes. A 1,000-step PlanB FP32 run reaches 0.437177 s with 10.631 m/s regular maximum, `3.85e-6 s^-1` RMS divergence, `5.27e-6 m^3/s` net flux, 26.28 ms last-step time, and 651.34 MiB persistent storage. The 259.38 m/s compact maximum is localized to a tiny trailing-edge/seam aperture and rises slowly, so bounded compact flux and grid/force convergence remain mandatory before the GUI/default switch. At 0.101029 s, FP32/FP64 streamwise total force agrees to about 2.7 ppm with a shared geometry condition cutoff of `1e4`.
