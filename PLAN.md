...
The imported-wing promotion gate is now connected behind `ExternalAeroExecutionOptions::conservative_cell_momentum` and exposed by an explicitly experimental checkbox in the GUI Run panel; the active mode is shown in live solver telemetry while the default remains staggered MacCormack. Its projection-to-cell feedback uses the aperture-area-weighted exact face-normal correction from the composite projection, not the historical raw pressure-traction impulse described above; this keeps persistent momentum compatible with projected MAC/aperture flux on skewed merged fragments. Provisional faces are rebuilt absolutely from the advanced control-volume state before projection. Retaining the prior projected face correction accumulated a face-only mode because a single collocated vector cannot represent every split aperture; absolute reconstruction removes that discrete null mode without a speed cap. Exact donor-rate CFL includes structured, EB, coarse/fine, and physical-boundary fluxes. A 1,000-step PlanB FP32 run reaches 1.958370 s with 12.301/12.656 m/s regular/compact maxima, `4.19e-6 s^-1` RMS divergence, `-4.78e-5 m^3/s` net flux, 32.65 ms last-step time, and 651.34 MiB persistent storage. At 200 steps / 0.352756 s, FP32/FP64 streamwise total force agrees to about 0.9 ppm. Coupled initialization and grid/domain/force convergence remain mandatory before making it the default.
