...
> file is an explicit experiment; it is unstable on high-incidence PlanB cases.
> Exact BVH fabric-side preprocessing remains enabled. Hash-based coarse/fine-link
> deduplication removed the former 75--85 second startup cost without weakening it.

This file records actual migration status, not a claim that unfinished architecture exists.

Historical experiment status: the GPU-resident collocated control-volume path implements structured/compact bounded MUSCL transport, same-topology molecular/Smagorinsky diffusion, implicit two-sided Spalding wall shear, pressure-compatible flux reconstruction, external flux boundaries, one composite AMR/EB projection, and projection-consistent pressure feedback. It remains available for manufactured research tests but is not the product default because imported-wing high-incidence stability is unresolved.
...
The production Spalding update is now implicit on each fluid control volume. Static preprocessing supplies the compact two-sided patch list and a deduplicated wall-CV map; each GPU step freezes the patch coefficients, sums their symmetric tangential drag tensors, and solves a three-component backward-Euler system only on that map. This removed the real-wing alternating-pressure/collapse mode caused by explicit overshoot when several patches shared a small CV. A stiff manufactured test verifies dissipation, no velocity reversal, CPU/GPU agreement, and exact fluid/fabric reaction. The remaining PlanB collapse was the independent explicit graph-diffusion stability limit at very short EB centre distances. Production now computes `max(sum(nu A/d)/V)` over structured, compact, EB, and coarse/fine connections and conservatively subcycles molecular/Smagorinsky diffusion at a 0.7 maximum-principle limit. A stiff regression is finite, dissipative, and momentum-conservative; the -6-degree imported wing needs seven substeps only during startup, returns to one, and remains bounded past the former 960--1050-step failure without a speed guard. The current full-wing case uses about 678 MiB; grid/domain/force convergence remains mandatory before aerodynamic use.
...
Historical collocated experiment: this path was temporarily promoted in `ExternalAeroExecutionOptions`, the GUI, and `paraglider_case_probe`, then demoted after high-incidence PlanB runs exposed an incompatible cell/face pressure mode. Its pressure-compatible reconstruction retained strong manufactured and low-incidence evidence, including FP32/FP64 parity and conservative compact transport, but did not establish imported-wing stability. It remains available explicitly for research without a speed cap or runaway guard. The production face-centred path retains exact BVH same-side preprocessing; hash-based link deduplication reduced its PlanB preprocessing from roughly 75--85 seconds to about 9 seconds without weakening the geometry test.
...
The 3-D overlay exposes step, physical CFD time, and Build-to-current/pause wall time. Single runs and automatic AoA-sweep cases use one tested exit policy. Besides the full-field settled decision, it compares time-weighted whole-wing force means in adjacent windows so a statistically stationary but instantaneously oscillating wake can complete. A configurable physical flow-through ceiling bounds any auto-paused run; that endpoint is labelled `MAX FLOW — NOT CONVERGED` and retains an explicitly labelled full or partial final-window mean rather than silently reporting one instantaneous load. An automatic AoA sweep uses the current CAD placement as the declared zero and rebuilds each pitched geometry. Results include an exit reason alongside D/L/L-over-D, optional CD/CL, simulation time, and wall time. Reference area is a prominent explicit input next to the sweep; it remains zero by default because flat versus projected paraglider area is a user convention, not something the solver may silently choose.
