...
The checked-in PlanB wing builds 4.48 million composite pressure states with zero unresolved owned topology cells; an initialized freestream projection reaches about a 9.4e-5 global relative residual in 168 iterations on the RTX 4090. A measured first following timestep costs about 27.1 ms advection, 8.5 ms LES/diffusion, 0.6 ms compact EB transport, and 141 ms projection (169 iterations; timings vary). Persistent storage including conservative 2:1 synchronization and compact EB transport is about 494.39 MiB. Its local maximum divergence and transient pressure-only force are not acceptance-quality; the current first-step PlanB force changes materially when EB transport is enabled. These numbers establish architecture and cost only, not aerodynamic validity.
