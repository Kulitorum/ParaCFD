...
Ordinary MacCormack/semi-Lagrangian sampling is invalid when a backtrace or interpolation stencil crosses fabric because a geometrically nearby point may be in a disconnected pressure/velocity region. Static preprocessing marks every MAC face centre within 2.5 local cells of fabric and tests its six one-cell Cartesian segments with the triangle BVH. Bits 0..5 of one byte store the open same-side links and bit 6 marks the fabric band. Marked faces use a bounded first-order donor update over only their open links and link-restricted molecular diffusion; all other faces use bounded MacCormack advection with RK2 characteristics, the finest-brick hash, and Smagorinsky diffusion. The global displacement is bounded below one cell, so the unmarked far-field stencil cannot reach the sheet. This prevents cross-fabric donor data without per-step triangle traversal, but the band still needs a higher-order same-side reconstruction and consistent wall/LES treatment. Same-level stencils cross brick boundaries instead of clamping, and every level's forward state is complete before the MacCormack correction reads it. At a 2:1 interface, compact normal-flux tiles gather from transported fine MAC faces and scatter projected values back to fine faces plus an aperture-weighted coarse mean. Irregular aperture velocities advance on a separate compact fluid-connectivity graph: adjacent implicit MAC faces and aperture states form the local fragment velocity, followed by first-order upwind transport and molecular diffusion. Neither graph contains a fabric crossing or imposes irregular indirection on far-field cells. General cross-level interpolation and higher-order/LES reconstruction on both graphs remain force-convergence blockers.
...
The checked-in PlanB wing builds 4.478 million composite pressure states with zero unresolved owned topology cells. GUI and probes canonicalize placement to one FP32-representable, base-brick-aligned frame before clipping; this removes topology changes caused by sub-micrometre display/CLI placement roundoff. At the tightened 1e-5 projection tolerance, the initialized freestream solve takes about 183 iterations and roughly 150-185 ms on the RTX 4090. With conservative same-side merging below 0.25 cell volume and removal/reporting below `1e-4 h^2`, 3,007 numerical slivers totalling 0.000312367 m^2 of cumulative level-atlas area are removed; a resolved opening remains ordinary fluid connectivity. A canonical-frame 500-step adaptive-CFL diagnostic with fabric-band same-side transport reaches 0.622 s. The regular peak stays near 12.1 m/s instead of growing to the previous 56.1 m/s; a compact EB aperture state remains near 46.0 m/s. Pressure-only force is still evolving at `[132.0, -0.57, 38.1]` N, max/volume-weighted-RMS divergence is `6.10e-4 / 3.90e-6 s^-1`, net integrated flux error is `-6.02e-5 m^3/s`, and the last step takes 70.1 ms including 35.6 ms projection. Persistent storage remains about 493.55 MiB because the same-side mask is one byte per face.

Resolution diagnostics can override only the level count through `paraglider_case_probe --max-levels N`. At two levels (0.125 m finest), the case has 1.316 million pressure states, uses 148.85 MiB, and after 300 steps at 1.362 s has regular/compact peaks of about 11.9/29.0 m/s. At four levels (0.03125 m finest), it has 12.938 million states, uses 1.427 GiB, and after 500 steps at 0.202 s has regular/compact peaks of 13.9/78.8 m/s; the last step is 203 ms with 95 ms projection. The three samples are at unequal physical times and therefore do not constitute force convergence. They do show that the regular path is bounded while the compact aperture path becomes more severe with refinement. Higher-order same-side compact transport, EB LES/wall treatment, and an explicit local velocity/flux acceptance criterion are mandatory before an equal-time force study can be meaningful.
