...
The checked-in PlanB wing builds 4.478 million composite pressure states with zero unresolved owned topology cells. At the tightened 1e-5 projection tolerance, the initialized freestream solve takes about 183 iterations and roughly 150-185 ms on the RTX 4090. With conservative same-side merging below 0.25 cell volume and removal/reporting below `1e-4 h^2`, 2,998 numerical slivers totalling 0.000311495 m^2 of cumulative level-atlas area are removed; a resolved opening remains ordinary fluid connectivity. A 500-step adaptive-CFL diagnostic with fabric-band same-side transport reaches 0.622 s. The regular peak settles near 12.0 m/s instead of growing to the previous 56.1 m/s; a compact EB aperture state remains near 46.0 m/s. Pressure-only force is still evolving at `[132.0, -0.58, 38.2]` N, max/volume-weighted-RMS divergence is `4.31e-4 / 3.90e-6 s^-1`, net integrated flux error is `1.17e-5 m^3/s`, and the last step takes 66.4 ms including 30.0 ms projection. Persistent storage remains about 493.55 MiB because the same-side mask is one byte per face. This is not an aerodynamic validation result; higher-order compact transport and force/grid convergence remain mandatory.
