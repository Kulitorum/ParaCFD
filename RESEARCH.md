...
The checked-in PlanB wing builds 4.478 million composite pressure states with zero unresolved owned topology cells. GUI and probes canonicalize placement to one FP32-representable, base-brick-aligned frame before clipping; this removes topology changes caused by sub-micrometre display/CLI placement roundoff. At the tightened 1e-5 projection tolerance, the initialized freestream solve takes about 183 iterations and roughly 150-185 ms on the RTX 4090. With conservative same-side merging below 0.25 cell volume and removal/reporting below `1e-4 h^2`, 3,007 numerical slivers totalling 0.000312367 m^2 of cumulative level-atlas area are removed; a resolved opening remains ordinary fluid connectivity. A canonical-frame 500-step adaptive-CFL diagnostic with graph-normal EB LES reaches 0.677 s. The regular peak is 13.1 m/s instead of the former 56.1 m/s structured leading-edge runaway, and the compact peak is 39.2 m/s. Pressure-only force is still evolving at `[131.9, -0.52, 39.4]` N, max/volume-weighted-RMS divergence is `7.74e-4 / 3.90e-6 s^-1`, net integrated flux error is `-6.79e-5 m^3/s`, and the last step takes 72.7 ms including 38.2 ms projection and 0.36 ms compact EB transport. Persistent storage remains about 493.55 MiB because the same-side mask is one byte per face. Reducing the merge threshold to 0.05 left a 65.7 m/s compact peak after 500 steps and advanced only 0.288 s, so 0.25 remains the default pending a less geometry-diffusive stabilization method.
